...
try {
  ...
  executeDueSchedules = scheduler.executeDueSchedules || scheduler.startLeadScheduler || null;
} catch (e) {
...
app.get('/', (_req, res) => res.json({ service: 'Agently WS Server — /ws + /realtime' }));
...
if (process.env.ENABLE_LEAD_SCHEDULER === 'true' && executeDueSchedules) {
  try {
    const intervalMs = parseInt(process.env.LEAD_SCHEDULER_INTERVAL_MS || '60000', 10);
...
try {
  ...
  console.warn('[WS] Billing tracker not available (copy billing-tracker.js to lib/):', e.message);
}
