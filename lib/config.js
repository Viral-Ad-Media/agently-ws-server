...
// API_URL / TWILIO_WS_URL self-heal.
// The Railway agently-ws-server service has been observed missing these two
// even though the repo .env has them, which disables the scheduler. Recover
// them from common alias vars, and derive TWILIO_WS_URL from Railway's own
// auto-injected RAILWAY_PUBLIC_DOMAIN when possible. We do NOT hardcode
// production URLs as silent defaults (that would mask real misconfig); we
// only pull from other env vars the platform already provides.
function ensureApiUrl() {
  const existing = String(process.env.API_URL || "").trim();
  if (existing) return existing;
  const alt = String(
    process.env.AGENTLY_API_URL ||
  ...
  if (!alt) return "";
  process.env.API_URL = alt;
  console.log("[config] API_URL recovered from alias env:", alt);
  return alt;
}

function ensureTwilioWsUrl() {
  const existing = String(process.env.TWILIO_WS_URL || "").trim();
  if (existing) return existing;
  // Derived only from an explicitly configured domain. The old
  // RAILWAY_PUBLIC_DOMAIN fallback is gone with the vendor — leaving it in
  // meant a stray Railway env var could silently route live calls back to a
  // service we no longer run.
  const domain = String(process.env.TWILIO_WS_DOMAIN || "").trim();
  if (!domain) return "";
  const derived = domain.startsWith("ws")
    ? domain.replace(/\/+$/, "")
    : `wss://${domain.replace(/^https?:\/\//, "").replace(/\/+$/, "")}`;
  process.env.TWILIO_WS_URL = derived;
  console.log("[config] TWILIO_WS_URL derived from TWILIO_WS_DOMAIN:", derived);
  return derived;
}
...
function validateRuntimeConfig() {
  ...
  const twilioWsUrl = normalizeUrlEnv("TWILIO_WS_URL", {
    protocol: "wss",
    expectedExample:
      "wss://agently-calls.zxy7w9w65bv9y.us-east-1.cs.amazonlightsail.com",
  });
  ...
  if (!validation.valid) {
    for (const err of validation.configErrors) {
      ...
      if (
        ...
        console.error(
          "[config] expected TWILIO_WS_URL=wss://agently-calls.zxy7w9w65bv9y.us-east-1.cs.amazonlightsail.com",
        );
      } else {
