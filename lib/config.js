...
function hasInlineGarbage(value) {
  return /\s/.test(value) || PLACEHOLDER_PATTERNS.some((pattern) => pattern.test(value));
}

function normalizeUrlEnv(name, { protocol, required = true, expectedExample } = {}) {
  const original = rawEnv(name);
  const trimmed = original.trim().replace(/\/+$/, "");
  const errors = [];

  if (!trimmed) {
    if (required) errors.push(`${name} is required${expectedExample ? `; expected ${name}=${expectedExample}` : ""}.`);
    return { name, value: "", valid: !required, errors, expectedExample: expectedExample || null };
  }

  if (original !== original.trim()) errors.push(`${name} has leading or trailing whitespace.`);
...
function validateRuntimeConfig() {
  ...
  return {
    ...
    schedulerEnabled: String(process.env.SCHEDULER_ENABLED || process.env.ENABLE_LEAD_SCHEDULER || "true").toLowerCase() !== "false",
  };
}

let lastLoggedSignature = "";
function logRuntimeConfigValidation(validation = validateRuntimeConfig()) {
  const signature = JSON.stringify({ valid: validation.valid, errors: validation.configErrors, apiUrl: validation.apiUrl, twilioWsUrl: validation.twilioWsUrl });
  if (signature === lastLoggedSignature) return validation;
  lastLoggedSignature = signature;

  if (validation.apiUrl) console.log("[config] API_URL normalized=", validation.apiUrl);
  if (validation.twilioWsUrl) console.log("[config] TWILIO_WS_URL normalized=", validation.twilioWsUrl);

  if (!validation.valid) {
    for (const err of validation.configErrors) {
      const name = err.split(" ")[0] || "config";
      if (/placeholder|spaces|comments|your-railway-app|YOUR-DOMAIN|optional/i.test(err)) {
        console.error("[config] invalid URL placeholder detected", err);
      } else if (name === "API_URL") {
        console.error("[config] invalid API_URL:", err);
        console.error("[config] expected API_URL=https://agently-server-v1.vercel.app");
      } else if (name === "TWILIO_WS_URL") {
        console.error("[config] invalid TWILIO_WS_URL:", err);
        console.error("[config] expected TWILIO_WS_URL=wss://agently-ws-server-production.up.railway.app");
      } else {
...
function requireValidRuntimeConfig() {
  ...
  if (!validation.valid) {
    const error = new Error(`Invalid runtime config: ${validation.configErrors.join(" ")}`);
