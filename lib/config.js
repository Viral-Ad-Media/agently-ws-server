...
function ensureTwilioWsUrl() {
  ...
  if (existing) return existing;
  // Railway always injects RAILWAY_PUBLIC_DOMAIN for a public service.
  const domain = String(
    process.env.TWILIO_WS_DOMAIN || process.env.RAILWAY_PUBLIC_DOMAIN || "",
  ).trim();
  if (!domain) return "";
  ...
  console.log("[config] TWILIO_WS_URL derived from Railway domain:", derived);
  return derived;
}
