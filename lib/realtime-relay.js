 ...
 * - Removed deprecated OpenAI-Beta header
 * - Updated event names to match GA API spec
 * - Updated session configuration structure
 *
 * WHY THIS EXISTS:
...
// GA Realtime API model (April 2026)
const REALTIME_URL =
  ...
  async connect() {
    return new Promise((resolve, reject) => {
      ...
      ws.on("open", () => {
        ...
        // Configure the session with our system prompt + settings
        // Updated structure for GA API (April 2026)
        ws.send(
          JSON.stringify({
            type: "session.update",
            session: {
              type: "realtime",
              output_modalities: ["text"], // text only — Twilio handles audio
