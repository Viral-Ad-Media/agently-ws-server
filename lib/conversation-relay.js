...
const LIVE_CALL_CREDIT_CHECK_INTERVAL_MS = Math.max(
  3000,
  Number(process.env.LIVE_CALL_CREDIT_CHECK_INTERVAL_MS || 10000),
);
...
async function handleConversationRelayWS(ws, req) {
  ...
  async function enforceLiveCallCredit(reason = "runtime-credit-check") {
    if (liveCreditStopRequested || state.endingCall) return false;
    ...
    try {
      ...
      console.warn("[credit-enforcement] conversation relay call stopped", {
        callSid,
        orgId,
        reason,
      ...
      if (ws.readyState === 1) {
        ws.send(
          JSON.stringify({
            type: "text",
            token: runtimeCreditStopMessage(status),
            last: false,
          }),
        );
        ws.send(JSON.stringify({ type: "text", token: "", last: true }));
        ws.close(1000, "billing credit required");
      }
      return true;
    } catch (err) {
      console.warn("[credit-enforcement] conversation relay check skipped", {
        callSid,
        orgId,
        reason,
  ...
  function startLiveCreditMonitor() {
    if (liveCreditCheckTimer || !orgId) return;
  ...
  void enforceLiveCallCredit("connection-open");
  ...
  ws.on("message", async (rawMsg) => {
    ...
    if (type === "setup") {
      ...
      void enforceLiveCallCredit("setup");
      return;
    }

    if (type === "prompt") {
      if (await enforceLiveCallCredit("prompt")) return;
      const callerText = msg.voicePrompt || "";
      if (!callerText.trim() || state.endingCall) return;
      const normalizedUtterance = callerText.trim().toLowerCase();
      const now = Date.now();
      if (state.pendingAssistantResponse) {
        console.log(
          "[CRelay WS] dropping overlapping prompt while assistant response is pending",
        );
        return;
      }
      if (
        normalizedUtterance === state.lastUserUtterance &&
        now - state.lastUserUtteranceAt < 2500
      ) {
        console.log("[CRelay WS] duplicate prompt ignored");
        return;
      }
      state.isUserSpeaking = false;
      state.pendingAssistantResponse = true;
      state.lastUserUtterance = normalizedUtterance;
      ...
      try {
        ...
        await generateStreamingResponse(messages, (token) => {
          fullReply += token;
          if (ws.readyState === 1 && !state.endingCall)
        ...
        state.pendingAssistantResponse = false;
        state.lastAssistantResponseAt = Date.now();
        ...
        if (
          /(goodbye|bye|that is all|that's all|no thanks|not interested|do not call|stop calling)/i.test(
            callerText + " " + fullReply,
          )
        ) {
          state.callObjectiveCompleted = true;
        }
      } catch (aiErr) {
    ...
    if (type === "speech_start") {
      ...
      state.lastUserUtteranceAt = Date.now();
      return;
    }
