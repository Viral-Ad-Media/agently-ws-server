...
async function debugCleanSlate({ organizationId, scheduleId } = {}) {
  const db = getSupabase();
  ...
  const { data: deletedRuns, error: runError } = await query.select(
  ...
  return {
    ...
    runsDeleted: (deletedRuns || []).length,
    schedulesDeleted: deletedSchedules.length,
    note: "Deleted only scheduled runs with no twilio_call_sid and no call_record_id.",
  };
}
