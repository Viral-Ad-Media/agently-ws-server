...
async function checkLimits(
  ...
  const concurrentCount = (concurrent || []).filter(
...
async function executeDueSchedules(options = {}) {
  const runOnce = options.runOnce === true;
  const startedAt = nowIso();
  schedulerState.lastTickAt = startedAt;
  schedulerState.lastError = null;
  if (runOnce) schedulerState.runOnceCount += 1;

  if (!schedulerEnabled()) {
    const summary = { skipped: true, reason: "disabled" };
  ...
  if (!config.valid) {
    ...
    schedulerState.lastError =
      config.configErrors.join(" ") || "Invalid config.";
    return summary;
  }
  if (runInProgress) {
    const summary = { skipped: true, reason: "already_running" };
    schedulerState.lastTickSummary = summary;
    return summary;
  }
  runInProgress = true;
  ...
  try {
    ...
    if (summary.dueRunsFound) {
      console.log("[scheduler] due queued runs found", {
        count: summary.dueRunsFound,
        runOnce,
      });
    }
    ...
    schedulerState.lastTickSummary = summary;
    if (summary.processed || runOnce)
      console.log("[scheduler] tick complete", summary);
    return summary;
  } catch (err) {
    const errorMessage = err.message || String(err);
    console.error("[scheduler] tick failed", errorMessage);
    summary.finishedAt = nowIso();
    summary.error = errorMessage;
    schedulerState.lastError = errorMessage;
    schedulerState.lastTickSummary = summary;
    return summary;
  } finally {
    runInProgress = false;
...
async function debugSchedule(scheduleId) {
  ...
  const [
    ...
    db
      .from("call_records")
      .select(
        "id,twilio_call_sid,status,duration,outcome,metadata,created_at,updated_at",
      )
  ...
  const recipients = (runs || []).map((run) => {
    const scheduledMs = run.scheduled_for
    ...
    if (String(run.status || "") !== "queued")
      whyNotDue = `status_${run.status || "unknown"}`;
    else if (!Number.isFinite(scheduledMs))
      whyNotDue = "missing_or_invalid_scheduled_for";
    else if (scheduledMs > nowMs) whyNotDue = "scheduled_for_future";
    ...
    return {
      ...
      providerUsed:
        ...
        null,
      concurrencyDecision: run.outcome_metadata?.deferred_reason || null,
    };
  });
  const counts = recipients.reduce((acc, run) => {
    const outcome = run.outcome || run.displayOutcome || "queued";
...
function debugSchedulerState() {
  const config = getSchedulerConfig();
  return {
    schedulerEnabled: schedulerEnabled() && config.valid,
    rawSchedulerEnabled: schedulerEnabled(),
    workerStarted: schedulerState.workerStarted,
    startupRequested: schedulerState.startupRequested,
    startedAt: schedulerState.startedAt,
    stoppedAt: schedulerState.stoppedAt,
    pollIntervalSeconds: schedulerState.pollIntervalSeconds,
    lastTickAt: schedulerState.lastTickAt,
    lastTickSummary: schedulerState.lastTickSummary,
    lastError: schedulerState.lastError,
    lastNotStartedReason: schedulerState.lastNotStartedReason,
    runInProgress,
    runOnceCount: schedulerState.runOnceCount,
    config: {
      apiUrlValid: config.apiUrlValid,
      twilioWsUrlValid: config.twilioWsUrlValid,
      configErrors: config.configErrors,
    },
  };
}
...
function startLeadScheduler() {
  schedulerState.startupRequested = true;
  schedulerState.pollIntervalSeconds = Math.max(
    15,
    intEnv("SCHEDULER_POLL_INTERVAL_SECONDS", 60),
  );
  ...
  console.log(
    ...
    schedulerState.pollIntervalSeconds,
  );
  if (!schedulerEnabled()) {
    schedulerState.workerStarted = false;
    schedulerState.lastNotStartedReason = "disabled_by_env";
    console.log("[scheduler] not started reason=disabled_by_env");
    return () => {};
  }
  if (!config.valid) {
    schedulerState.workerStarted = false;
    schedulerState.lastNotStartedReason = "invalid_config";
    schedulerState.lastError =
      config.configErrors.join(" ") || "Invalid config.";
    console.error("[scheduler] disabled invalid config");
    console.error("[scheduler] config invalid, leaving runs queued", {
  ...
  const intervalMs = schedulerState.pollIntervalSeconds * 1000;
  schedulerState.workerStarted = true;
  schedulerState.startedAt = nowIso();
  schedulerState.stoppedAt = null;
  schedulerState.lastNotStartedReason = null;
  ...
  return () => {
    ...
    schedulerTimer = null;
    schedulerState.workerStarted = false;
    schedulerState.stoppedAt = nowIso();
  };
}
