...
function hasRealTwilioSid(row) {
  return String(row?.twilio_call_sid || "").trim().length > 0;
}

function hasCallRecordLink(row) {
  return String(row?.call_record_id || "").trim().length > 0;
}

function countsAsRealScheduledCall(row) {
  return hasRealTwilioSid(row) || hasCallRecordLink(row);
}

function noCallLinkedFilter(row) {
  return !hasRealTwilioSid(row) && !hasCallRecordLink(row);
}

function callRecordTerminal(record) {
  if (!record) return false;
  const status = String(record.status || "").toLowerCase();
  const outcome = String(record.outcome || "").toLowerCase();
  const metadata = record.metadata || {};
  const endedReason = String(
    metadata.ended_reason || metadata.endReason || "",
  ).toLowerCase();
  return (
    [
      "completed",
...
async function loadLinkedCallRecords(db, runs = []) {
  const ids = [
    ...new Set((runs || []).map((row) => row.call_record_id).filter(Boolean)),
  ];
  const sids = [
    ...new Set(
      (runs || [])
  ...
  for (let i = 0; i < ids.length; i += chunkSize) {
    ...
    const { data, error } = await db
      .from("call_records")
      .select("id,status,outcome,twilio_call_sid,metadata,started_at,timestamp")
      .in("id", chunk);
  ...
  for (const record of records) {
    ...
    const sid = String(record?.twilio_call_sid || "").trim();
    if (sid) bySid.set(sid, record);
  }
  return { byId, bySid };
}

async function getActiveConcurrencyDiagnostics(
  db,
  organizationId,
  maxAllowed = null,
) {
  ...
  const { data, error } = await db
    ...
    .select(
      "id,schedule_id,status,twilio_call_sid,call_record_id,started_at,target_name,destination_phone",
    )
  ...
  const candidates = (data || []).filter(countsAsRealScheduledCall);
  const { byId, bySid } = await loadLinkedCallRecords(db, candidates);
  const countedRuns = candidates.filter((row) => {
    const sid = String(row.twilio_call_sid || "").trim();
    const record =
      (row.call_record_id ? byId.get(row.call_record_id) : null) ||
      (sid ? bySid.get(sid) : null) ||
      null;
    // If the call record says the call already ended, do not hold a
    // concurrency slot for the rest of the active window. This fixes tests
    // where completed/voicemail calls kept blocking later scheduled calls.
    return !callRecordTerminal(record);
  });
  ...
  return {
    ...
    countedRuns: countedRuns.map((row) => {
      const sid = String(row.twilio_call_sid || "").trim();
      const record =
        (row.call_record_id ? byId.get(row.call_record_id) : null) ||
        (sid ? bySid.get(sid) : null) ||
      ...
      return {
        ...
        callRecordStatus: record?.status || null,
        callRecordOutcome: record?.outcome || null,
        startedAt: row.started_at || null,
        targetName: row.target_name || null,
        destinationPhone: row.destination_phone || null,
      };
    }),
    ignoredEndedRunIds: candidates
      .filter((row) => {
        const sid = String(row.twilio_call_sid || "").trim();
        const record =
          (row.call_record_id ? byId.get(row.call_record_id) : null) ||
...
async function checkLimits(
  ...
  const [
    ...
    getActiveConcurrencyDiagnostics(db, organizationId, maxConcurrent),
    db
      .from("lead_outreach_runs")
      .select("id,twilio_call_sid,call_record_id,started_at,status")
      .eq("organization_id", organizationId)
  ...
  const concurrentCount = activeConcurrency.countedRunIds.filter(
    (id) => !currentRunId || id !== currentRunId,
  ).length;
  const orgMinuteCount = (orgMinute || []).filter(
    countsAsRealScheduledCall,
  ).length;
  const globalMinuteCount = (globalMinute || []).filter(
    countsAsRealScheduledCall,
  ).length;
  const orgDailyCount = (orgDaily || []).filter(
    countsAsRealScheduledCall,
  ).length;
  const scheduleDailyCount = (scheduleDaily || []).filter(
    countsAsRealScheduledCall,
  ).length;

  const diagnostics = {
    activeConcurrency: { ...activeConcurrency, activeCount: concurrentCount },
    usage: {
      orgMinuteCount,
      globalMinuteCount,
      orgDailyCount,
  ...
  if (orgMinuteCount >= maxOrgPerMinute)
    return {
      ok: false,
      code: "ORG_RATE_LIMIT",
  ...
  if (globalMinuteCount >= maxGlobalPerMinute)
    return {
      ok: false,
  ...
  if (scheduleDailyCount >= maxScheduleDaily)
    return {
      ok: false,
  ...
  return { ok: true, limits, ...diagnostics };
}
...
async function resetStaleNoSidRuns(db) {
  ...
  const { data: staleRuns, error: selectError } = await db
    .from("lead_outreach_runs")
    .select("id,twilio_call_sid,call_record_id,started_at,status")
    .eq("status", "initiated")
  ...
  if (selectError) {
    console.warn("[scheduler] stale no-sid reset skipped", {
      error: selectError.message || String(selectError),
    });
    return 0;
  }
  const ids = (staleRuns || []).filter(noCallLinkedFilter).map((row) => row.id);
...
async function debugSchedule(scheduleId) {
  ...
  const activeConcurrency = schedule?.organization_id
    ? await debugActiveConcurrency({ organizationId: schedule.organization_id })
    : null;
  return {
    ok: true,
    serverNowUtc: serverNow.toISOString(),
...
async function debugActiveConcurrency({ organizationId } = {}) {
  const db = getSupabase();
  ...
  if (!targetOrgId) {
    return {
      ...
      countedRunIds: [],
      countedRuns: [],
      note: "Pass organizationId to include active concurrency diagnostics.",
    };
  }
  ...
  return getActiveConcurrencyDiagnostics(
    db,
    targetOrgId,
    limits.maxOrgConcurrentCalls,
  );
}
...
async function debugCleanSlate({
  ...
  let query = db
    .from("lead_outreach_runs")
    .select(
      "id,schedule_id,status,twilio_call_sid,call_record_id,error_code,started_at,created_at",
    )
    .in("status", [
      "queued",
      "initiated",
  ...
  const matchedRuns = (candidates || []).filter(noCallLinkedFilter);
  const ids = matchedRuns.map((run) => run.id).filter(Boolean);
  const scheduleIds = [
    ...new Set(matchedRuns.map((run) => run.schedule_id).filter(Boolean)),
  ];
  ...
  if (dryRun) {
    return {
      ok: true,
      dryRun: true,
      runsMatched: ids.length,
      runsDeleted: 0,
      linkedRunsReset: resetLinked ? 0 : undefined,
      schedulesTouched: scheduleIds.length,
      schedulesDeleted: 0,
      note: "Dry run only. Matched scheduled runs with no twilio_call_sid and no call_record_id.",
    };
  }
  ...
  if (resetLinked) {
    const olderThan = new Date(
      Date.now() -
        Math.max(1, Number(resetLinkedOlderThanMinutes) || 2) * 60_000,
    ).toISOString();
    let linkedQuery = db
      .from("lead_outreach_runs")
    ...
    const { data: linkedCandidates, error: linkedSelectError } =
      await linkedQuery;
    if (linkedSelectError) throw linkedSelectError;
    const linkedIds = (linkedCandidates || [])
      .filter(countsAsRealScheduledCall)
      .map((run) => run.id)
      .filter(Boolean);
    if (linkedIds.length && !dryRun) {
      const { data, error } = await db
        .from("lead_outreach_runs")
        .update({
          status: "completed",
          ...
          error_message: null,
          outcome_metadata: {
            resetBy: "debug_clean_slate",
            resetReason: "manual_test_reset_of_linked_initiated_runs",
            resetAt: nowIso(),
          },
        })
        .in("id", linkedIds)
        .select("id,schedule_id,status");
      if (error) throw error;
      resetLinkedRuns = data || [];
    } else if (linkedIds.length && dryRun) {
      resetLinkedRuns = linkedIds.map((id) => ({ id }));
    }
  }
  ...
  console.warn("[debug/clean-slate] deleted no-call scheduled test rows", {
    organizationId: targetOrgId || null,
    scheduleId: targetScheduleId || null,
    runsMatched: ids.length,
    runsDeleted: deletedRuns.length,
    linkedRunsReset: resetLinkedRuns.length,
    schedulesTouched: scheduleIds.length,
  ...
  return {
    ...
    schedulesTouched: scheduleIds.length,
    schedulesDeleted: deletedSchedules.length,
    note: "Deleted only scheduled runs with no twilio_call_sid and no call_record_id. Twilio-linked calls were preserved.",
  };
}
