...
async function processJob(db, job) {
  ...
  // Reclaim stale "scraping" rows as well as fresh work.
  //
  // Job leases expire so a crashed worker's JOB is picked up again, but the
  // page and source rows it had already flipped to "scraping" were not in
  // this filter — so they were never retried and never cleared. Nine sources
  // sat in "scraping" for up to seven weeks with no job behind them, which
  // made re-scraping those sites silently do nothing. A row still marked
  // "scraping" but untouched for longer than the lease has no live worker on
  // it by definition, so it is safe to take.
  const staleBefore = new Date(Date.now() - LEASE_SECONDS * 1000 * 2).toISOString();
  const { data: pages, error: pagesErr } = await db
    .from("knowledge_discovered_pages")
    .select("id,url,normalized_url,title,content_hash,scrape_status,updated_at")
    .eq("discovery_id", job.discovery_id)
    .eq("organization_id", job.organization_id)
    .eq("is_selected", true) // issue 4(c): ONLY selected pages, discard the rest
    .or(
      `scrape_status.in.(pending,queued,failed),and(scrape_status.eq.scraping,updated_at.lt.${staleBefore})`,
    )
