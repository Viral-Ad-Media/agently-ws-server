...
function normalizePurposeTypos(text) {
  return normalizeSpaces(text)
    .replace(/\bseasn\s+greetins\b/gi, "seasonal greetings")
    .replace(/\bseason\s+greet(?:ing)?s?\b/gi, "seasonal greetings")
    .replace(/\bfollowup\b/gi, "follow-up")
    .replace(/\bfollow\s+up\b/gi, "follow-up")
    .replace(/\breachout\b/gi, "reach out")
    .replace(/\bwell\s*being\b/gi, "well-being")
    .replace(/\bwellbeing\b/gi, "well-being")
    .replace(
      /\bhealth\s+and\s+customer\s+follow\s*-?\s*up\b/gi,
      "health and customer follow-up",
    )
    .replace(/\bcust(?:omer)?\s+follow\s*-?\s*up\b/gi, "customer follow-up")
    .replace(/\s+/g, " ")
    .trim();
}

function isBareTopicPhrase(text) {
  const value = normalizeSpaces(text);
  if (!value) return false;
  if (/^(?:to|about|regarding|concerning|because|for)\b/i.test(value))
    return false;
  if (
    /\b(?:please|ask|tell|say|confirm|schedule|book|collect|remind|invite|explain|offer|send|call|reach|follow|check|verify)\b/i.test(
      value,
    )
  )
    return false;
...
function humanizeOutboundPurposeForSpeech(text, maxChars = 220) {
  let purpose = sanitizeOutboundPurposeText(
    normalizePurposeTypos(text),
    maxChars,
  );
  if (!purpose) return "follow up briefly";

  const lower = purpose.toLowerCase();

  if (
    /\b(monthly|routine|regular)\b/.test(lower) &&
    /\b(well-being|health|customer follow-up)\b/.test(lower)
  ) {
    return "check in on your well-being, share a quick health follow-up, and make sure everything is going well";
  }

  if (/\bwell-being\b/.test(lower) && /\bseasonal greetings?\b/.test(lower)) {
    return "check in on your well-being and share seasonal greetings";
  }

  if (/\bwell-being\b/.test(lower) && /\bcheck\b/.test(lower)) {
    return "check in on your well-being";
  }

  if (/\bcustomer follow-up\b/.test(lower) && /\bhealth\b/.test(lower)) {
    return "share a quick health and customer follow-up";
  }

  if (/^monthly outreach\s+(?:on|about|regarding)\s+/i.test(purpose)) {
    return purpose.replace(
      /^monthly outreach\s+(?:on|about|regarding)\s+/i,
      "do a quick monthly check-in about ",
    );
  }

  if (/^outreach\s+(?:on|about|regarding)\s+/i.test(purpose)) {
    return purpose.replace(
      /^outreach\s+(?:on|about|regarding)\s+/i,
      "check in about ",
    );
  }

  if (/^customer follow-up$/i.test(purpose))
    return "follow up with you as a customer";
  if (/^health$/i.test(purpose))
    return "check in briefly about your health and wellness";
  if (/^well-being$/i.test(purpose)) return "check in on your well-being";

  if (isBareTopicPhrase(purpose)) {
    purpose = purpose
      .replace(/^monthly outreach\b/i, "a quick monthly check-in")
      .replace(/^outreach\b/i, "a quick check-in")
      .replace(/\bcustomer follow-up\b/gi, "customer follow-up")
      .trim();
  }
...
function outboundGreetingReasonClause(text) {
  const purpose = humanizeOutboundPurposeForSpeech(text, 180);
  ...
  if (
    /^follow\s*-?\s*up\s+(?:with\s+you\s+)?(?:about|regarding|on)\s+/i.test(
      purpose,
    )
  ) {
    return `I'm following up about ${purpose.replace(/^follow\s*-?\s*up\s+(?:with\s+you\s+)?(?:about|regarding|on)\s+/i, "")}`;
  }
  ...
  if (/^[a-z]+ing\b/i.test(purpose)) return `I'm calling to ${purpose}`;
  if (
...
function buildOutboundGreeting({
  ...
  return `${hello}, this is ${cleanAgent} from ${cleanOrganization}. ${reason}. Do you have a quick moment to talk?`;
}
...
function repairOutboundPurposePhrasing(text) {
  let output = normalizePurposeTypos(text)
    .replace(
    ...
    .replace(
      /\b(I\s*(?:am|'m)\s+)?calling\s+because\s+for\b/gi,
      "I'm calling to",
    )
    .replace(
    ...
    .replace(/\bbecause\s+to\b/gi, "to")
    .replace(/\bbecause\s+for\b/gi, "about")
    .replace(/\breachout\b/gi, "reach out")
  ...
  output = output.replace(
    /\bI'm calling to monthly outreach on well-being, health and customer follow-up\b/gi,
    "I'm calling to check in on your well-being, share a quick health follow-up, and make sure everything is going well",
  );
  output = output.replace(
    /\bI'm calling about monthly outreach on well-being, health and customer follow-up\b/gi,
    "I'm calling to check in on your well-being, share a quick health follow-up, and make sure everything is going well",
  );
  output = output.replace(
    /\bI'm calling to well-being check and seasonal greetings\b/gi,
    "I'm calling to check in on your well-being and share seasonal greetings",
  );
  output = output.replace(
    /\bI'm calling to monthly outreach on well-being, health and customer followup\b/gi,
    "I'm calling to check in on your well-being, share a quick health follow-up, and make sure everything is going well",
  );
  output = output.replace(
    /\bI'm calling about monthly outreach on well-being, health and customer followup\b/gi,
    "I'm calling to check in on your well-being, share a quick health follow-up, and make sure everything is going well",
  );
  if (/share a quick health follow-up/i.test(output)) {
    output = output.replace(
      /\?\s*is now a good time\??/i,
      ". Is now a good time?",
    );
  } else {
    output = output.replace(
      /\bI'm calling to ([^.?!]{3,160})\??\s+is now a good time\??/gi,
      (_m, purpose) =>
        `${outboundGreetingReasonClause(purpose)}. Is now a good time?`,
    );
  }
  return output;
}
...
function outboundBehaviorRules({ callPurpose = "", recipientName = "" } = {}) {
  const cleanPurpose = humanizeOutboundPurposeForSpeech(callPurpose, 320);
