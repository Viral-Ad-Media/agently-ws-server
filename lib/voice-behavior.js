...
function collapseDuplicatePhrases(text) {
  ...
  for (let i = 0; i < 3; i += 1) {
    for (const pattern of phrasePatterns) output = output.replace(pattern, "$1");
  }
  return output;
}
...
function removeInternalPurposePrefixes(text) {
  let value = normalizeSpaces(text)
    .replace(/^[-–—:;,.\s]+/, "")
    .replace(/^(?:hi|hello)?\s*(?:this\s+is\s+[^,.]{1,80}\s+from\s+[^,.]{1,80}\s+(?:and\s+)?)?(?:i|we)\s*(?:am|are|'m|'re)?\s*(?:calling|reaching\s*out|contacting|phoning)\s+from\s+[^,.]{1,80}\s+(?:because|to|about|regarding|concerning|for|in\s+order\s+to)\s+/i, "")
...
function sanitizeOutboundPurposeText(text, maxChars = 180) {
  ...
  value = value
    ...
    .replace(/\babout\s+customer\s+well-being\b/gi, "about your well-being as a customer")
...
function addRecipientPossessiveWhenNatural(text) {
  ...
  if (/^(appointment|booking|consultation|enrollment|order|payment|account|subscription|webinar|call|session)\b/i.test(value)) {
    return `your ${value}`;
  }
  return value;
}
...
function outboundGreetingReasonClause(text) {
  const purpose = sanitizeOutboundPurposeText(text, 180);
  ...
  if (/^follow\s+up\s+(?:with\s+you\s+)?(?:about|regarding|on)\s+/i.test(purpose)) {
  ...
  if (/^(check|confirm|schedule|reschedule|follow|ask|remind|notify|inform|discuss|update|invite|verify|collect|share|see|help|offer|introduce|review|explain|request|complete|book|arrange)\b/i.test(purpose)) {
...
function safeCustomOutboundGreeting(text) {
  ...
  if (/\bwho\s+(?:is\s+this|am\s+i\s+speaking\s+with)\b/i.test(value)) return "";
  return value;
}
...
function repairOutboundPurposePhrasing(text) {
  return normalizeSpaces(text)
    ...
    .replace(/\b(I\s*(?:am|'m)\s+)?calling\s+to\s+inquire\s+about\s+you\s+are\s+calling\s+to\b/gi, "I'm calling to")
...
function removeInboundHelpdeskPhrasesForOutbound(text) {
  let output = asString(text);
  const helpPhrase = /\b(?:how|what)\s+(?:can|may)\s+i\s+help\s+you(?:\s+with)?\s*(?:today|right\s+now|this\s+(?:morning|afternoon|evening))?\??/gi;
  ...
  output = output.replace(/\s+hello\s+this\s+is\s+[^.!?]{1,80}(?:[.!?]|\s*[-–—]\s*)?$/i, "");
  output = output.replace(/(^|[.!?]\s+)hello\s+this\s+is\s+[^.!?]{1,80}(?:[.!?]|\s*[-–—]\s*)/gi, "$1");
...
function outboundBehaviorRules({ callPurpose = "", recipientName = "" } = {}) {
  ...
  return [
    ...
    cleanPurpose ? `- Recipient-facing call reason: ${cleanPurpose}` : "- If the purpose is missing, say you are calling to follow up briefly.",
