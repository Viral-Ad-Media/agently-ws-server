...
class RealtimeSession {
  ...
  async connect() {
    return new Promise((resolve, reject) => {
      ...
      this.ws = ws;
      const connectTimeout = setTimeout(() => reject(new Error('Realtime WS connect timeout')), 8000);
...
// ─────────────────────────────────────────────────────────────
async function createRealtimeSession({ systemPrompt, onText, onDone, onError }) {
  const apiKey = (process.env.OPENAI_API_KEY || '').trim();
  if (!apiKey) {
    console.warn('[Realtime] OPENAI_API_KEY not set — skipping Realtime API, using fallback');
    return null;
  }

  const session = new RealtimeSession({ systemPrompt, onText, onDone, onError, apiKey });
  try {
    await session.connect();
    console.log('[Realtime] Session connected ✅');
    return session;
  } catch (err) {
    console.warn('[Realtime] Failed to connect, falling back to standard API:', err.message);
    return null;
  }
}
