...
function rankFaqs(message, faqs, limit = 10) {
  ...
  // Same bug fix as assistant-intelligence.js: don't discard everything when
  // nothing scores above zero here; fall back to the incoming order instead.
...
function baseBehaviorInstructions({
  ...
  const noKnowledgeLine =
    context.stats?.chunks || context.stats?.faqs
      ...
      : "No scoped website knowledge or FAQs were loaded for this business yet. Never say this to the customer directly (see GRACEFUL FALLBACK RULES) — identify yourself normally and quietly capture the question for the team.";
...
- The provided knowledge may not use the customer's exact wording (typos, abbreviations, synonyms). Before concluding something isn't covered, mentally match their words against anything topically close in what's provided. Treat a close, clearly-related match as a real answer, not a non-match.
...
LISTS AND PRODUCTS:
- If asked for a list, answer with the actual specific items by name from the knowledge, not just a category summary, using a real bulleted/numbered list.
- When a specific product/service/page is matched and has a URL, include it as a markdown link: [Product Name](url). If the user wants to buy/checkout, give that specific link, not the homepage.

GRACEFUL FALLBACK RULES (never sound incompetent):
- NEVER say "I don't have that in my knowledge base," "that's not available," "not in the knowledge base," or anything exposing internal systems/databases to the customer.
- If the topic is clearly related to this business but there's no exact match, help with the closest genuinely relevant thing available, framed as an answer, not an apology.
- Only when a request is genuinely unrelated should you redirect — warmly, e.g. "That's a great one for our team to confirm directly — let me grab your details so someone can follow up." Always offer to take contact details when redirecting this way.
- Never answer with bare uncertainty alone; always pair any limitation with a concrete next step.
...
function compactTranscriptText(transcriptLines = [], max = 2200) {
  return (transcriptLines || [])
    .map(
      (line) =>
        `${line.role || line.speaker || "speaker"}: ${line.text || line.transcript || ""}`,
    )
    .join("\n")
    .slice(0, max);
}
...
function extractStructuredCallInsights({
  ...
  const transcript = compactTranscriptText(transcriptLines, 4000);
  const userText = (transcriptLines || [])
    .filter((line) =>
      /caller|user|recipient|customer|human/i.test(
        String(line.role || line.speaker || ""),
      ),
    )
    .map((line) => String(line.text || line.transcript || "").trim())
    .filter(Boolean)
    .join(" ");
  ...
  let callbackTime = "";
  const callbackMatch = allText.match(
    /(?:call\s+me\s+back|call\s+back|callback|reach\s+me|contact\s+me)(?:[^.?!\n]{0,80})/i,
  );
  if (callbackMatch) callbackTime = callbackMatch[0].trim();

  let messageForTeam = "";
  const messageMatch = allText.match(
    /(?:tell\s+(?:them|the\s+team)|let\s+(?:them|the\s+team)\s+know|message\s+is|leave\s+(?:a\s+)?message)(?:[^\n]{0,240})/i,
  );
  if (messageMatch) messageForTeam = messageMatch[0].trim();
  if (!messageForTeam && callbackTime) messageForTeam = callbackTime;
  if (
    !messageForTeam &&
    /not interested|do not call|don't call|remove me|unsubscribe/i.test(allText)
  ) {
    messageForTeam =
  ...
  // Preferred signal: the live search_business_knowledge tool (see
  // twilio-media-stream.js handleSearchKnowledgeTool) already tells us
  // exactly which caller queries came back with nothing relevant, completely
  // independent of how the model then phrased its spoken reply. Use the
  // first miss as the representative unanswered question for this call.
  const searchMissQueries = Array.isArray(context.searchToolMissQueries)
    ? context.searchToolMissQueries.filter(Boolean)
    : [];
  if (searchMissQueries.length) {
    unansweredQuestion = String(searchMissQueries[0]).slice(0, 300);
  } else {
    // Fallback (search tool wasn't available/invoked this call, e.g. no KB
    // assigned): the old phrase-based heuristic. Kept as a safety net.
    const questionMatch = userText.match(
      /[^.?!]*(?:what|why|how|when|where|who|can|do|does|is|are|will)[^.?!]*\?/i,
    );
    if (
      questionMatch &&
      /do not have|don't have|not sure|team member|follow up|cannot answer|can't answer/i.test(
        allText,
      )
    ) {
      unansweredQuestion = questionMatch[0].trim();
    }
  }
  ...
  const summary =
    ...
    (transcriptLines.length
      ? `Call completed with ${transcriptLines.length} captured transcript turn(s).${messageForTeam ? ` Action item: ${messageForTeam}` : ""}`
      : "Call completed. No caller transcript was captured.");

  return {
    summary: String(summary).slice(0, 1800),
    outcome:
      /not interested|remove me|unsubscribe|do not call|don't call/i.test(lower)
        ? "opt_out_or_not_interested"
        : messageForTeam
          ? "message_or_callback_requested"
          : transcriptLines.length
            ? "completed"
    ...
    collected_fields: {
      ...
      unanswered_questions_all: searchMissQueries.slice(0, 10),
      caller_phone: context.callerPhone || context.recipientPhone || "",
      opt_out: /do not call|don't call|remove me|unsubscribe/i.test(lower),
      knowledge_search_hits: Number(context.searchToolHits || 0),
      knowledge_search_misses: Number(context.searchToolMisses || 0),
    },
  };
}
...
function buildVoiceIntelligencePrompt({
  ...
  const businessName =
    ...
    organization?.name ||
    voiceContext?.organization?.name ||
    "the business";
  const agentName = agent?.name || context.agentName || "the assistant";
  const rawPurpose = context.callPurpose || context.customInstructions || "";
  const purpose =
    isOutbound && voiceBehavior.interpretOutboundPurposeForSpeech
      ? voiceBehavior.interpretOutboundPurposeForSpeech(rawPurpose)
      : voiceBehavior.sanitizeOutboundPurposeText(rawPurpose, 260);
  const recipient = voiceBehavior.cleanRecipientNameForSpeech(
    context.recipientName || context.targetName || context.leadName || "",
  );

  if (!isOutbound) {
    return [
      "LIVE CALL INTELLIGENCE LAYER — INBOUND:",
      `- You are ${agentName} answering calls for ${businessName}.`,
      "- The caller contacted the business; greet like a receptionist and ask how you can help.",
      "- Listen first. Do not talk over the caller. Use the knowledge base for answers.",
      "- If the caller needs follow-up, take a concise message and callback/contact details.",
      "- Before ending, ask whether there is anything else and then close politely.",
    ].join("\n");
  }

  return [
    "LIVE CALL INTELLIGENCE LAYER — OUTBOUND:",
    `- You are ${agentName} calling on behalf of ${businessName}. We called the recipient; do not act like they called us.`,
    recipient
      ? `- Known recipient name: ${recipient}. Use it naturally in the opening and closing, but do not overuse it.`
      : "- No safe recipient name is available. Say 'Hello' only. Never say 'Outbound Lead' or other placeholders.",
    purpose
      ? `- Interpreted call intent: ${purpose}. Use this meaning, not the raw operator wording.`
      : "- No clear purpose was provided. Say you are calling to follow up briefly.",
    rawPurpose && rawPurpose !== purpose
      ? `- Raw operator note: ${rawPurpose}. Do not read this verbatim.`
      : "",
    "- Opening structure: greet, introduce yourself and the organization, explain the reason in one natural sentence, then ask if now is a quick moment.",
    "- Never say 'I am calling because for...' or any mechanically combined phrase. Rewrite broken operator text into normal human speech.",
    "- Be warm, patient, and professional. A brief natural hesitation such as 'sure', 'okay', or 'mm-hmm' is allowed only when it helps the conversation; do not overuse filler.",
    "- After the opening, stop and listen. If the recipient interrupts, stop speaking immediately and let them finish.",
    "- If they ask why you are calling, acknowledge calmly, explain the reason once, and offer to keep it brief, schedule a callback, take a message, or end the call.",
    "- If they ask a business question, answer only from the loaded knowledge base. If unsure, take a message instead of guessing.",
    "- Save actionable messages, callback requests, opt-outs, complaints, or unanswered questions using the capture_inbound_message tool. Save only the actionable note, not the entire transcript.",
    "- Before ending, confirm whether they have any other question or message. Then thank them by name if known, say goodbye, and let the backend end the call.",
  ]
    .filter(Boolean)
    .join("\n");
}
