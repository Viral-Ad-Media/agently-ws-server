...
function extractCallbackTime(text) {
  ...
  for (const pattern of patterns) {
    ...
    if (raw) return raw.replace(/^\s*(at|around|by|after|before)\s+/i, "").trim().slice(0, 120);
  }
  return "";
}
...
function detectInterest(text) {
  ...
  if (/\b(yes|interested|sounds good|tell me more|send|call me|book|schedule|appointment|i want|let'?s do|go ahead)\b/.test(t)) return "interested";
  if (/\b(maybe|not sure|later|another time|think about|send information)\b/.test(t)) return "maybe";
  if (/\b(no|not interested|stop calling|do not call|don'?t call|remove me|unsubscribe|decline)\b/.test(t)) return "not_interested";
  return "unknown";
}

function detectOptOut(text) {
  return /\b(stop calling|do not call|don'?t call|remove me|unsubscribe|take me off|opt out|wrong number)\b/i.test(asString(text));
}

function detectTransferRequest(text) {
  return /\b(human|representative|agent|manager|someone else|transfer|speak to a person|real person)\b/i.test(asString(text));
}

function detectAppointmentInterest(text) {
  return /\b(appointment|book|schedule|meeting|consultation|demo|call me back|callback|follow up)\b/i.test(asString(text));
}

function detectObjection(text) {
  const line = lastMatchingLine(text.split("\n").map((t) => ({ text: t })), (value) =>
    /\b(expensive|price|cost|busy|not interested|already have|too much|can'?t afford|later|no time|don'?t need|skeptical|not sure)\b/i.test(value),
  );
...
function detectQuestion(text) {
  const lines = asString(text)
    ...
    .map((line) => line.trim())
    .filter(Boolean);
  const match = lines.find((line) => /\?|\b(what|how|when|where|why|can you|do you|is there|are there|price|cost)\b/i.test(line));
  return match ? match.slice(0, 500) : "";
}

function assistantCouldNotAnswer(assistantText) {
  return /\b(i don'?t have|i do not have|not have enough information|cannot answer|can'?t answer|team can follow up|take a message|not in.*knowledge)\b/i.test(asString(assistantText));
}

function extractStructuredCallInsights({ transcriptLines = [], context = {}, existingSummary = "" } = {}) {
  ...
  const unansweredQuestion = questionAsked && assistantCouldNotAnswer(assistantText) ? questionAsked : "";
  ...
  const phone = extractPhone(callerText, firstNonEmpty(context.recipientPhone, context.callerPhone));
  const requestedFollowUp = Boolean(callbackTime || /\b(call me|call back|callback|follow up|reach me)\b/i.test(callerText));
  const completedByUserApproval = /\b(yes|yeah|okay|ok|sure|go ahead|you can|that's all|that is all)\b.{0,60}\b(end|hang up|bye|goodbye|finish)\b/i.test(callerText);
  const callerEnded = /\b(bye|goodbye|thank you|thanks|that'?s all|that is all)\b/i.test(callerText);
  const summaryParts = [];
  ...
  if (optOut) summaryParts.push("Caller requested opt-out/do-not-call handling.");
  if (transferRequested) summaryParts.push("Caller requested a human/transfer.");
  return {
    ...
    collected_fields: {
      ...
      caller_ended_or_approved_end: Boolean(completedByUserApproval || callerEnded),
    },
    flags: {
...
function buildVoiceIntelligencePrompt({ context = {}, agent = {}, organization = {}, voiceContext = {} } = {}) {
  const isOutbound = String(context.direction || "").toLowerCase() === "outbound";
  const callPurpose = firstNonEmpty(context.callPurpose, voiceContext?.debug?.currentCall?.callPurpose);
  const customInstructions = firstNonEmpty(context.customInstructions, voiceContext?.debug?.currentCall?.customInstructions);
  ...
  const lines = [
    ...
    "- Do not interrupt or talk over the caller. If interrupted, stop and listen.",
    ...
    "- Capture useful details when naturally offered: name, phone, email, interest level, question, objection, callback request, appointment interest, transfer request, opt-out request, and message for the team.",
    "- If the caller asks you to pass a message to the team, repeat the message back once briefly for confirmation.",
    ...
    "- End-call rule: when the objective appears complete, ask permission before ending: 'That completes this call. Is it okay if I end the call now?' End only after the caller confirms, unless the caller clearly says goodbye or Twilio/system ends the call.",
  ];
  if (isOutbound) {
    lines.push("- This is an outbound call. The reason for the call is high priority. Mention it naturally and briefly.");
  }
  if (callPurpose) lines.push(`- Current call purpose: ${callPurpose}`);
  if (customInstructions) lines.push(`- Operator custom instructions for this call: ${customInstructions}`);
  return lines.join("\n");
}
