...
async function resolveAssignedKnowledgeBase(
  ...
  const ignoredLinkRows = await safeQuery(
  ...
  diagnostics.knowledgeBaseAssignment =
    "none: direct voice_agents.knowledge_base_id is empty";
  if ((ignoredLinkRows || []).length) {
    diagnostics.ignoredLegacyKbLinkCount = ignoredLinkRows.length;
  }
  return "";
}
