...
// General, tenant-agnostic safety net used by every voice agent's local
// re-ranking (chooseRelevantChunks/chooseRelevantLinks), complementing the
// DB-side trigram/full-text ranking used for the primary knowledge-chunk
// retrieval below. Bounded/short-circuited so a correctly-spelled query never
// pays the extra cost.
...
function scoreText(text, keywords) {
  ...
  for (const keyword of keywords) {
    ...
    // for every tenant/business since this is shared ranking code.
    if (!haystackWords) {
      haystackWords = [
...
function inferRuntimeBusinessFacts({
  ...
  add(
    "If a business-specific detail is missing from the selected KB, do not say so directly to the caller — help with the closest relevant thing available, or warmly offer to take a message/schedule a callback so someone can confirm the exact detail.",
  );
  return facts;
}
...
async function resolveAssignedKnowledgeBase(
  ...
  // FINAL FALLBACK: agent has no direct KB and no link. Rather than load NO
  // knowledge (which makes the agent invent answers and fail basic questions
  // like the business name/website), use the organization's primary knowledge
  // base — strictly org-scoped, so tenant isolation is preserved. Correct
  // multi-business setups still resolve via the direct column / link above.
  const orgKbRows = await safeQuery(
    "organization primary knowledge base fallback",
    () =>
      db
        .from("knowledge_bases")
        .select("id,is_primary,status,updated_at")
        .eq("organization_id", organizationId)
        .order("is_primary", { ascending: false })
  ...
  const orgKbId = asString(orgKbRows?.[0]?.id);
  if (orgKbId) {
    diagnostics.knowledgeBaseAssignment =
      "organization primary knowledge base (agent had no direct KB or link)";
    return orgKbId;
  }
  diagnostics.knowledgeBaseAssignment =
...
function buildTwilioVoicePrompt({
  ...
  const parts = [
    ...
    // Knowledge is scraped from the business's own website, so it reads in the
    // third person ("Abokirate provides…", "visit abokirate.com"). Agents were
    // copying that voice back to callers — "you can go to their website" —
    // which makes them sound like an outsider describing the company rather
    // than the company answering its own phone.
    `- You WORK FOR ${businessName}. You are one person on their team, not the company itself and not an outside describer.`,
    `- Speak about YOURSELF as "I": "I can check that for you", "let me look", "I'll take a message". You are ${agentName}, an individual.`,
    `- Speak about the BUSINESS as "we" and "our": "our opening hours", "we offer", "our website". Never "they", "them", "their", and never the company name as though it were somebody else.`,
    `- So: "I can help with that — our price for it is X." Not "we can help with that" for your own action, and not "their price".`,
    "- The knowledge below is written about the business from the outside. Convert it as you speak. Do not read it back verbatim.",
    isOutbound
...
- If that custom prompt contains inbound and outbound sections, follow ONLY the inbound section for this call and ignore conflicting outbound language.`
    ...
    "- The caller's wording may not match the knowledge exactly (typos, mishearing, abbreviations). Before treating something as unavailable, mentally match it against anything topically close in what's provided below — a close, clearly-related match counts as a real answer.",
    '- NEVER say "I don\'t have that information," "that\'s not available," "not in my knowledge base," or anything exposing internal systems/databases. If something is genuinely unrelated to this business, warmly say a team member will follow up with the exact answer, and take a message (see MESSAGE, CONTACT, AND LEAD HANDLING below) — do not just say you lack the information.',
    "LISTS AND PRODUCT LINKS:",
    "- If asked for a list (products, services, options), answer with the actual specific items by name from the knowledge below, not just a category summary.",
    "- If the caller wants to buy, order, or checkout a specific item and a URL for it exists in the knowledge, offer to text/send that specific link rather than a generic homepage. Do not read raw URLs aloud; refer to the page/product name and offer to send the link.",
    ...(selectedKnowledgeBaseId
      ? [
          "LIVE LOOKUP TOOL:",
          "- You have a search_business_knowledge tool. Use it whenever the caller asks about something specific (a product, price, policy, availability, detail) that isn't already fully covered by what's already in this prompt, or if you're not fully certain — better to check than to guess or generalize.",
          "- Call it silently mid-conversation; never announce that you're searching or checking anything. Keep talking naturally; a brief natural pause is fine while it runs.",
          "- Use the caller's own words as the query. After it returns, answer using those results the same way you would answer from the knowledge already provided above — same rules apply (real names, lists, links, never mention a knowledge base).",
          "- If it comes back empty, do not tell the caller you searched or found nothing — follow the GRACEFUL FALLBACK RULES above.",
        ]
      : []),
    ...
    "- If a specific product/service genuinely is not found after the fuzzy-matching check above, warmly offer the closest available alternatives from the knowledge instead of saying it wasn't found.",
...
${profileLines}`);
    ...
    parts.push(
      "NO DIRECT KNOWLEDGE BASE ASSIGNED (internal note, never say this to the caller):\nThis agent has no value in voice_agents.knowledge_base_id. Do not use organization, onboarding, chatbot, old link-table, or unscoped FAQ/chunk content as business knowledge. If asked for specific business facts, do not mention any assignment/configuration issue to the caller — warmly say a team member will follow up with the exact details, and take a message.",
    );
  }
  if (scheduleText)
...
${unresolvedQuestionText}`);
  ...
  if (!faqText && !knowledgeText) {
    parts.push(
      "NO SPECIFIC KNOWLEDGE LOADED (internal note, never say this to the caller):\nYou can identify yourself as the business assistant using organization and agent details. For specific product/service/policy questions, do not mention missing information — warmly say a team member will follow up with the exact details, and take a message.",
    );
  }
  parts.push(buildFormattingRules());
  return parts.join("\n\n").slice(0, 26000);
...
async function loadVoiceContext(db, orgId, agentRow, query, extra = {}) {
  ...
  const [
    ...
    selectedKnowledgeBaseId
      ...
      : Promise.resolve([]),
    // Primary chunk retrieval now goes through search_knowledge_chunks, the
    // same Postgres function the text chatbot uses (full-text + trigram
    // ranking, typo-tolerant). This used to be a raw fetch of up to 500 rows
    // re-scored locally with exact-substring-only matching, meaning voice
    // never benefited from the trigram fallback at all. Falls back to the
    // previous raw-fetch behavior if the RPC is ever unavailable, so
    // chooseRelevantChunks()'s local (now also fuzzy-tolerant) scoring still
    // applies as a safety net either way.
    selectedKnowledgeBaseId
      ? safeQuery(
          "knowledge_chunks via search_knowledge_chunks rpc",
          async () => {
            const { data, error } = await db.rpc("search_knowledge_chunks", {
              p_organization_id: organizationId,
              p_knowledge_base_ids: [selectedKnowledgeBaseId],
              p_query: asString(query),
              p_limit: 24,
              // 1500 clipped facts that sit late in a chunk — measured
              // occurrences of a live figure ran to character 1693.
              p_max_chars: 1800,
            });
            if (error) throw error;
            if (Array.isArray(data) && data.length) return { data };
            // RPC returned nothing (e.g. truly empty KB) — no need to fall
            // back further, an empty KB is an empty KB either way.
            return { data: data || [] };
          },
          null, // sentinel: null (not []) tells us the RPC path failed below
          diagnostics,
        )
      : Promise.resolve([]),
  ]);

  // If the RPC path failed outright (returned our null sentinel), fall back
  // to the original raw fetch + local fuzzy-tolerant scoring rather than
  // silently losing all knowledge chunks for the call.
  let orgChunkRowsResolved = orgChunkRows;
  if (orgChunkRowsResolved === null && selectedKnowledgeBaseId) {
    orgChunkRowsResolved = await safeQuery(
      "knowledge_chunks by selected knowledge_base_id (rpc fallback)",
      () =>
        db
          .from("knowledge_chunks")
          .select("*")
  ...
  const normalizedOrgChunks = (orgChunkRowsResolved || [])
    .map(normalizeChunk)
    .filter((chunk) => {
  ...
  // NOTE: mirrors the voiceProfile computation inside buildTwilioVoicePrompt()
  // (lines ~555-573). That function only returns the prompt string, not this
  // value, so it must be recomputed here for the debug.agent.voiceProfile
  // field below. Previously this referenced an out-of-scope `voiceProfile`
  // and threw a ReferenceError, which sent this whole function into its
  // caller's catch block and discarded all loaded FAQs/knowledge/products.
  const voiceProviderForDebug = asString(
    agentRow?.voice_provider,
    "",
  ).toLowerCase();
  const voiceProfile =
    voiceProviderForDebug === "elevenlabs"
      ? asString(
          agentRow?.elevenlabs_voice_name ||
            agentRow?.elevenlabs_voice_id ||
            agentRow?.voice_id ||
