...
function normalizeChunk(row) {
  return {
    ...
    chunkIndex: Number.isFinite(Number(row?.chunk_index)) ? Number(row.chunk_index) : 0,
...
function sourceLooksUploaded(chunk) {
  const src = `${chunk.sourceUrl || ""} ${chunk.sourceTitle || ""}`.toLowerCase();
  if (!chunk.sourceUrl) return true;
  return /upload|uploaded|document|file|pdf|docx|storage|supabase|bucket/.test(src) && !/^https?:\/\//i.test(chunk.sourceUrl);
}
...
function looksProductService(text) {
  return /product|service|sell|buy|price|pricing|package|plan|order|shop|store|supplement|gumm|wellness|delivery|shipping|booking/i.test(asString(text));
}
...
function buildChatPrompt({ businessName, customPrompt, faqs, chunks, links, collectLeads }) {
  ...
  parts.push("Answer using only the information relevant to the user's request. Do not dump unrelated FAQs or unrelated website text.");
  parts.push("When the user is specific, give the exact answer or direct page link. When the user is broad or there are several plausible matches, present the best options and ask one short clarifying question so the user can choose.");
  parts.push("If the exact requested item or page is not available, say so plainly and then offer the closest valid alternatives.");
  if (collectLeads) parts.push("If the request cannot be fully resolved, ask for contact details so the team can follow up.");
  if (customPrompt) parts.push(`CUSTOM INSTRUCTIONS:\n${customPrompt}`);
  if ((faqs || []).length) parts.push("FAQS:\n" + faqs.map((faq) => `Q: ${faq.question}\nA: ${faq.answer}`).join("\n\n"));
  if ((chunks || []).length) parts.push("RELEVANT WEBSITE KNOWLEDGE:\n" + chunks.map((chunk, index) => `[Source ${index + 1}] ${chunk.content}`).join("\n\n---\n\n"));
  if ((links || []).length) parts.push("RELEVANT PAGES:\n" + links.map((link) => `- ${link.label}: ${link.url}`).join("\n"));
...
function buildTwilioVoicePrompt({ agentRow, organization, faqs, relevantChunks, linkedChatbots, direction = "inbound", assignmentContext = "" }) {
  ...
  const greeting = asString(agentRow?.greeting, "Hello, thank you for calling. How can I help you today?");
  ...
  const captureFields = asArray(agentRow?.data_capture_fields).length
    ? asArray(agentRow.data_capture_fields).map((item) => asString(item)).filter(Boolean).join(", ")
    : "name, phone, email, reason";
  const callPurposes = asArray(agentRow?.call_purposes).map((item) => asString(item)).filter(Boolean);
  const rules = agentRow?.rules && typeof agentRow.rules === "object" ? JSON.stringify(agentRow.rules) : asString(agentRow?.rules);
  const links = chooseRelevantLinks(relevantChunks || [], "business pages products services support contact shop", 10);
  const faqText = (faqs || []).slice(0, 60).map((faq) => `Q: ${faq.question}\nA: ${faq.answer}`).join("\n\n");
  const knowledgeText = (relevantChunks || [])
    .slice(0, 24)
    .map((chunk, index) => {
      const title = chunk.sourceTitle || chunk.sourceUrl || `Knowledge ${index + 1}`;
...
async function loadChatbotContext(db, chatbotId, query) {
  const chatbot = await safeQuery("chatbot", () =>
    db
      .from("chatbots")
  ...
  const rawChunks = await safeQuery("chatbot knowledge chunks", async () => {
    const base = await db
...
async function loadVoiceContext(db, orgId, agentRow, query, extra = {}) {
  ...
  const [orgFaqRows, orgChatbots, orgChunkRows] = await Promise.all([
    safeQuery("faqs by organization_id", () =>
      db
        .from("faqs")
        .select("*")
    ...
    safeQuery("chatbots by organization_id", () =>
      db
        .from("chatbots")
        .select("*")
        .eq("organization_id", organizationId)
    ...
    safeQuery("knowledge_chunks by organization_id", () =>
      db
        .from("knowledge_chunks")
        .select("*")
        .eq("organization_id", organizationId)
  ...
  const linkedChatbots = (orgChatbots || []).filter((bot) =>
  ...
  const linkedChatbotIds = new Set(linkedChatbots.map((bot) => asString(bot.id)).filter(Boolean));

  const agentFaqs = normalizeFaqs((orgFaqRows || []).filter((row) => !row.voice_agent_id || sameId(row.voice_agent_id, agentId)));
  const chatbotFaqs = [];
  for (const bot of linkedChatbots) chatbotFaqs.push(...normalizeFaqs(asArray(bot.faqs)));
  const faqs = uniqueBy([...agentFaqs, ...chatbotFaqs], (f) => `${f.question}|${f.answer}`).slice(0, 80);
  if (faqs.length === 0) {
  ...
  const normalizedOrgChunks = (orgChunkRows || []).map(normalizeChunk).filter((chunk) => chunk.content && sameId(chunk.organizationId, organizationId));
  const exactAgentChunks = normalizedOrgChunks.filter((chunk) => sameId(chunk.voiceAgentId, agentId));
  const chatbotChunks = normalizedOrgChunks.filter((chunk) => linkedChatbotIds.has(asString(chunk.chatbotId)));
  const orgFallbackChunks = normalizedOrgChunks.filter((chunk) => !chunk.voiceAgentId && !chunk.chatbotId);
  ...
  if (selectedChunks.length === 0 && normalizedOrgChunks.length > 0) {
    selectedChunks = uniqueBy(normalizedOrgChunks, (chunk) => `${chunk.sourceUrl}|${chunk.content.slice(0, 180)}`);
    diagnostics["knowledge selection"] = "no exact voice_agent_id/linked chatbot/null chunks; used all organization_id chunks as tenant-safe fallback";
  } else if (selectedChunks.length === 0) {
    diagnostics["knowledge selection"] = diagnostics["knowledge_chunks by organization_id"] || "no rows for organization_id";
  }
  ...
  const uploadedDocumentChunks = relevantChunks.filter(sourceLooksUploaded).length;
  const scrapedContent = relevantChunks.filter(sourceLooksScraped).length;
  const productsServices = relevantChunks.filter((chunk) => looksProductService(`${chunk.sourceTitle}\n${chunk.content}`)).length;
  const stats = {
    faqs: faqs.length,
  ...
  if (stats.knowledgeChunks === 0) diagnostics.knowledgeChunks = diagnostics["knowledge selection"] || "no tenant knowledge rows loaded";
  if (stats.uploadedDocumentChunks === 0) diagnostics.uploadedDocumentChunks = stats.knowledgeChunks ? "loaded chunks did not look like uploaded document chunks" : diagnostics.knowledgeChunks;
  if (stats.scrapedContent === 0) diagnostics.scrapedContent = stats.knowledgeChunks ? "loaded chunks did not have http/https source_url" : diagnostics.knowledgeChunks;
  if (stats.productsServices === 0) diagnostics.productsServices = stats.knowledgeChunks ? "no loaded chunk matched product/service keywords" : diagnostics.knowledgeChunks;
  if (stats.callPurposes === 0) diagnostics.callPurposes = "voice_agents.call_purposes is empty";

  console.log("[context-builder] loaded faqs count", stats.faqs, stats.faqs === 0 ? diagnostics.faqs : "");
  console.log("[context-builder] loaded knowledge chunks count", stats.knowledgeChunks, stats.knowledgeChunks === 0 ? diagnostics.knowledgeChunks : "");
  console.log("[context-builder] loaded scraped pages count", stats.scrapedContent, stats.scrapedContent === 0 ? diagnostics.scrapedContent : "");
  console.log("[context-builder] loaded uploaded chunks count", stats.uploadedDocumentChunks, stats.uploadedDocumentChunks === 0 ? diagnostics.uploadedDocumentChunks : "");
  console.log("[context-builder] loaded products/services count", stats.productsServices, stats.productsServices === 0 ? diagnostics.productsServices : "");
  console.log("[context-builder] final prompt chars", stats.finalPromptChars);

  const firstFaq = faqs[0] ? `Q: ${faqs[0].question}\nA: ${faqs[0].answer}` : "";
  const firstKnowledgeChunk = relevantChunks[0] ? truncate(relevantChunks[0].content, 500) : "";
  const firstProductServiceChunk = relevantChunks.find((chunk) => looksProductService(`${chunk.sourceTitle}\n${chunk.content}`));
  ...
  return {
    ...
    samples: {
      firstFaq,
      firstKnowledgeChunk,
      firstProductService: firstProductServiceChunk ? truncate(firstProductServiceChunk.content, 500) : "",
    },
    debug: {
      agent: {
        id: agentRow.id || "",
