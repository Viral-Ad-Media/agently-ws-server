...
async function resolveAssignedKnowledgeBase(
  ...
  const linkRows = await safeQuery(
    "agent_knowledge_base_links by voice_agent_id",
    () =>
      db
  ...
  if (linkId)
    diagnostics.knowledgeBaseAssignment = "agent_knowledge_base_links";
  return linkId;
}
...
- If that custom prompt contains inbound and outbound sections, follow ONLY the inbound section for this call and ignore conflicting outbound language.`
  ...
  if (selectedKnowledgeBaseId) {
    const profileLines = [
      `Knowledge Base name: ${businessName}`,
...
async function loadVoiceContext(db, orgId, agentRow, query, extra = {}) {
    ...
    safeQuery(
      ...
      () => {
        let q = db
          ...
          .eq("organization_id", organizationId);
        if (selectedKnowledgeBaseId)
          q = q.eq("knowledge_base_id", selectedKnowledgeBaseId);
        return q.limit(160);
      },
      [],
      diagnostics,
  ...
  if (selectedKnowledgeBaseId) {
    ...
    if (selectedChunks.length === 0 && normalizedOrgChunks.length > 0) {
      ...
      diagnostics["knowledge selection"] =
        "no exact voice_agent_id/linked chatbot/null chunks; used all organization_id chunks because no Knowledge Base is assigned";
