...
function buildTwilioVoicePrompt({
  ...
  const purposeText = isOutbound ? asString(callPurpose) : "";
  ...
  const parts = [
    ...
    isOutbound
      ? "- Start warmly using the known recipient name when available, briefly say who you are, state the reason for the call in one concise sentence, then wait."
    ...
    isOutbound && purposeText
      ? `- Primary outbound call purpose/reason for reaching out: ${purposeText}`
      : "",
    ...
    "- If the caller gives a message or callback request, repeat it back once briefly for confirmation.",
