...
function buildTwilioVoicePrompt({
  ...
  const directRecipientText = directRecipient
    ? [
        directRecipient.name
          ? `Direct recipient name: ${directRecipient.name}`
          : "",
  ...
  const parts = [
    ...
    customPrompt ? `- High-priority custom prompt: ${customPrompt}` : "",
    ...
    "MESSAGE AND LEAD DETAIL COLLECTION:",
    "- If the caller asks something you cannot answer, collect the question/message and offer follow-up.",
    "- Capture caller name, phone, email, interest level, objection, requested callback, appointment interest, transfer request, opt-out request, and message for the team when naturally offered.",
    ...
    "- If the caller gives a message or callback request, repeat it back once briefly to confirm.",
    "- Use the capture_inbound_message tool/function when a caller asks for follow-up, leaves a message, asks an unanswered question, or provides callback/contact details. This applies to inbound and outbound calls.",
    ...
    "- When the objective is complete, ask: That completes this call. Is it okay if I end the call now?",
    "- End only after the caller confirms, unless the caller clearly says goodbye or Twilio/system ends the call.",
