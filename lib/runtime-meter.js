...
function railwayRuntimeUnitCostUsd() {
  ...
  let rate = 0;
  try {
    const raw = process.env.RAILWAY_RATE_CARD_JSON;
    if (raw) {
      ...
      const match = rows.find(
        (r) =>
          r &&
          String(r.provider || "").toLowerCase() === "railway" &&
    ...
    console.warn(
      "[runtime-meter] RAILWAY_RATE_CARD_JSON is set but did not parse:",
      err?.message || String(err),
    );
  }
  if (!rate) {
    console.warn(
      "[runtime-meter] no Railway runtime rate resolved; runtime usage will price at $0. Set RAILWAY_RATE_CARD_JSON.",
    );
  }
  ...
  async function write(eventType, seconds, reason = null, extra = {}) {
    try {
      return await logRailwayRuntimeUsage({
        ...
        // Without an explicit unit cost the event lands with
        // unit_cost_usd = null, internal cost resolves to 0, and the 70%
        // target-margin rule then computes 0 / 0.30 = $0.00 — which is why
        // every web call billed nothing. Resolve the Railway runtime rate
        // here so the existing margin logic has a cost to work from.
        unitCostUsd: railwayRuntimeUnitCostUsd(),
        callId: callId || null,
