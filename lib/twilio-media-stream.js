...
function isOutboundRefusalOrDone(text) {
  const t = normalizeSpeechText(text);
  if (!t) return false;
  if (hasHangupCancelIntent(t)) return false;
  return [
    /\b(not interested|no thanks|no thank you)\b/,
    /\b(i'?m good|i am good|i'?m okay|i am okay|i'?m fine|i am fine)\b/,
    /\b(i'?m done|i am done|done|that'?s all|that is all|nothing else|not really|not at the moment|not now)\b/,
    /^(no|nope|nah|okay|ok|alright|all right|sure|bye|goodbye|good bye|thanks|thank you)\.?$/,
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  function maybeSendEarlyPreparedGreeting(reason = "twilio-start") {
    ...
    if (!elevenLabsVoice?.voiceId) {
      elevenLabsVoice = {
        ...
        name: context.elevenLabsVoiceNameHint || context.voiceProfile || "",
    ...
    console.log("[latency] early_greeting_started_at", {
      callSid: context.callSid,
      streamSid,
      earlyGreetingStartedAt: new Date().toISOString(),
  ...
  function requestHangupConfirmation(reason = "caller-end-intent") {
    ...
    if (
      ...
      scheduleFinalAckHangup("already_closing", reason);
      return;
    }
    ...
    if (hangupConfirmationAttempts >= 1) {
      console.log(
        "[hangup] confirmation already asked once; ending without asking again",
        {
          callSid: context.callSid,
          streamSid,
          reason,
      ...
      requestClosingAndHangup(
        "caller-end-intent-after-confirmation-loop",
        CLOSING_MESSAGE,
      );
      return;
    }
      ...
      if (type === "conversation.item.input_audio_transcription.completed") {
        ...
        if (transcript) {
          ...
          if (
            ...
            requestClosingAndHangup(
              "outbound_refusal_or_done",
              finalClosingMessage("outbound_refusal_or_done"),
            );
            return;
          }
          ...
          if (awaitingHangupConfirmation) {
            ...
            if (
              ...
              requestClosingAndHangup(
                "caller-end-intent",
                finalClosingMessage("caller-end-intent"),
              );
            } else {
  ...
  twilioWs.on("message", async (raw) => {
    ...
    if (message.event === "start") {
      ...
      console.log("[latency] twilio_start_received_at", {
        callSid: context.callSid,
        streamSid,
        twilioStartReceivedAt: new Date(twilioStartReceivedAt).toISOString(),
      });
