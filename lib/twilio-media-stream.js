...
const OPENAI_REALTIME_MODEL =
  process.env.OPENAI_REALTIME_MODEL || "gpt-realtime";
const OPENAI_REALTIME_SESSION_SHAPE = String(
  process.env.OPENAI_REALTIME_SESSION_SHAPE || "auto",
)
  .trim()
  .toLowerCase();

function shouldUseCurrentRealtimeSessionShape() {
  if (
    ["ga", "current", "new", "realtime"].includes(OPENAI_REALTIME_SESSION_SHAPE)
  )
    return true;
  if (["legacy", "beta", "old"].includes(OPENAI_REALTIME_SESSION_SHAPE))
    return false;
  return (
    /(^|[?&=/_-])gpt-realtime([?&=/_-]|$)/i.test(OPENAI_REALTIME_URL) ||
    /^gpt-realtime/i.test(OPENAI_REALTIME_MODEL)
  );
}

function chooseRealtimeSessionUpdate({
  useCurrentShape,
  voiceProvider,
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  let openaiCurrentSessionShapeSent = false;
  ...
  async function ensureOpenAIInternal() {
    ...
    openaiWs.on("open", () => {
      ...
      openaiCurrentSessionShapeSent = shouldUseCurrentRealtimeSessionShape();
      safeSend(
        openaiWs,
        chooseRealtimeSessionUpdate({
          useCurrentShape: openaiCurrentSessionShapeSent,
          voiceProvider,
          scheduledSystemPrompt,
          selectedVoice,
          messageCaptureEnabled,
      ...
      logLifecycle("OpenAI session.update sent", {
        ...
        sessionShape: openaiCurrentSessionShapeSent ? "current-ga" : "legacy",
      });
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "error") {
        ...
        // Realtime deployments can disagree on session shape during migrations.
        // If OpenAI rejects the first session.update, try the opposite shape once
        // instead of leaving the caller unheard.
        if (
          !openaiSessionFallbackSent &&
          /session\.type|missing required parameter|input_audio_format|output_audio_format|unknown parameter|invalid|beta api|realtime beta api|session object/i.test(
            errMsg,
          )
        ) {
          openaiSessionFallbackSent = true;
          const fallbackUsesCurrentShape = !openaiCurrentSessionShapeSent;
          openaiCurrentSessionShapeSent = fallbackUsesCurrentShape;
          safeSend(
            openaiWs,
            chooseRealtimeSessionUpdate({
              useCurrentShape: fallbackUsesCurrentShape,
              voiceProvider,
              scheduledSystemPrompt,
              selectedVoice,
              messageCaptureEnabled,
          ...
          logLifecycle("OpenAI alternate session fallback sent", {
            callSid: context.callSid,
            streamSid,
            error: errMsg,
            sessionShape: fallbackUsesCurrentShape ? "current-ga" : "legacy",
          });
        }
        return;
      }
