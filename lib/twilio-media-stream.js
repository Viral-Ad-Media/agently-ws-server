...
function useCurrentRealtimeSessionShape() {
  const configured = String(process.env.OPENAI_REALTIME_SESSION_SHAPE || "")
    .trim()
    .toLowerCase();
  if (["ga", "current", "realtime"].includes(configured)) return true;
  if (["legacy", "beta"].includes(configured)) return false;
  const model = String(process.env.OPENAI_REALTIME_MODEL || "gpt-realtime");
  return /^gpt-realtime/i.test(model);
}

function audioFormatForCurrentRealtime() {
  // OpenAI Realtime GA accepts a format object without a sample-rate field.
  // Twilio Media Streams are 8kHz mu-law, represented as g711_ulaw.
  return { type: "g711_ulaw" };
}
...
function realtimeSessionUpdateCurrent(
  ...
  return {
    ...
    session: {
      ...
      audio: {
        input: {
          format: audioFormatForCurrentRealtime(),
          transcription: {
            model:
...
function buildTextResponse(instructions) {
  return {
    ...
    response: useCurrentRealtimeSessionShape()
      ? {
          output_modalities: ["text"],
          instructions,
        }
...
function buildGreetingResponse(greeting) {
  return buildOpenAiAudioResponse(
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function ensureOpenAIInternal() {
    ...
    openaiWs.on("open", () => {
      ...
      const initialSessionPayload = useCurrentRealtimeSessionShape()
        ? voiceProvider === "elevenlabs"
      ...
      safeSend(openaiWs, initialSessionPayload);
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "error") {
        ...
        if (
          ...
          /input_audio_format|output_audio_format|response\.modalities|session\.type|unknown parameter|missing required parameter|invalid|beta api|realtime beta api|session object/i.test(
            errMsg,
          )
        ) {
          ...
          safeSend(openaiWs, fallbackPayload);
          logLifecycle("OpenAI GA session fallback sent", {
            callSid: context.callSid,
            streamSid,
            error: errMsg,
            sessionShape: "current-ga",
          });
        }
        return;
      }
