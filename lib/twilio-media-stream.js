...
function sanitizeOutboundPurposeText(text, maxChars = 150) {
  ...
  value = firstSentence
    .replace(
      /^(?:i\s+am|i'm|we\s+are|we're)\s+(?:calling|reaching\s+out)\s+(?:because|to|about|regarding|for)\s+/i,
      "",
    )
    .replace(/^this\s+call\s+is\s+(?:for|about|regarding)\s+/i, "")
    .replace(/^for\s+the\s+purpose\s+of\s+/i, "")
    .replace(
      /^(?:the\s+)?purpose\s+(?:of\s+this\s+call\s+)?(?:is\s+|was\s+|of\s+|to\s+)?/i,
      "",
    )
...
function outboundGreetingReasonClause(text) {
  const purpose = sanitizeOutboundPurposeText(text, 150);
  if (!purpose) return "I'm calling to follow up";
  ...
  if (
    /^(check|confirm|schedule|reschedule|follow|ask|remind|notify|inform|discuss|update|invite|verify|collect|share|see|reach|help|offer|introduce|review|explain)\b/i.test(
      purpose,
    )
  ) {
...
function buildOutboundGreeting({
  ...
  const cleanRecipient = firstNonEmpty(recipientName);
  const cleanAgent = firstNonEmpty(agentName, "your assistant");
  const cleanOrganization = firstNonEmpty(organizationName, "the business");
  ...
  return `${hello}, this is ${cleanAgent} from ${cleanOrganization}. ${reason}. Is now a good time?`;
}
...
function speakerForTranscriptRole(role) {
  const normalized = String(role || "")
    .trim()
    .toLowerCase();
  if (["caller", "user", "human", "recipient", "customer"].includes(normalized))
    return "Caller";
  if (["assistant", "agent", "bot", "ai"].includes(normalized)) return "Agent";
  if (normalized === "transcription") return "Transcription";
  return "Unknown";
}
...
function buildTranscriptArray(lines) {
  return (lines || []).map(normalizeTranscriptLine).filter(Boolean);
}
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    const recipientNameForGreeting = firstNonEmpty(
      context.recipientName,
      context.targetName,
      currentCallDebugForGreeting.directRecipientName,
      currentCallDebugForGreeting.leadName,
    );
    const outboundGreeting = repairOutboundPurposePhrasing(
      firstNonEmpty(
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  let lastTranscriptSaveKey = "";
  let finalTranscriptSaveStarted = false;
  ...
  function transcriptSaveKey() {
    return JSON.stringify(
      buildTranscriptArray(transcriptLines).map((line) => [
        line.role,
        line.text,
      ]),
    );
  ...
  function queueTranscriptSnapshotSave(
    reason = "transcript-update",
    delayMs = 250,
  ) {
    if (!context.callRecordId && !context.callSid) return;
    ...
    transcriptSaveTimer = setTimeout(
      () => {
        ...
        const key = transcriptSaveKey();
        if (!key || key === lastTranscriptSaveKey) return;
        lastTranscriptSaveKey = key;
        const snapshot = transcriptLines.slice();
        ...
        transcriptSaveChain = transcriptSaveChain
          .then(() =>
            saveTranscriptOnly({
              ...
              summary: summarizeTranscript(snapshot, 900),
              reason,
            }),
          )
          .catch((err) => {
            console.warn("[transcript] incremental save failed", {
              callSid: context.callSid,
              streamSid,
              reason,
  ...
  async function flushTranscriptSnapshotSave(reason = "final") {
    ...
    if (transcriptLines.length) {
      const key = transcriptSaveKey();
      if (key && key !== lastTranscriptSaveKey) {
        lastTranscriptSaveKey = key;
        const snapshot = transcriptLines.slice();
        const snapshotContext = { ...context };
        refreshVoiceOutputDebug();
        transcriptSaveChain = transcriptSaveChain.then(() =>
          saveTranscriptOnly({
            context: snapshotContext,
            transcriptLines: snapshot,
            summary: summarizeTranscript(snapshot, 900),
    ...
    try {
      ...
      console.warn("[transcript] final save chain failed", {
        callSid: context.callSid,
        streamSid,
        reason,
        error: err.message || String(err),
  ...
  async function finishMessageCapture() {
    if (finalTranscriptSaveStarted) {
      try {
        await transcriptSaveChain;
        await captureSaveChain;
      } catch (_) {}
      return captureState.saveResult || null;
    }
    finalTranscriptSaveStarted = true;
    try {
      await captureSaveChain;
    } catch (_) {}
    refreshVoiceOutputDebug();
    await flushTranscriptSnapshotSave("call-finish");
  ...
  async function speakWithElevenLabs(text, label = "assistant") {
    ...
    const queuedJob = async () => {
      ...
      try {
        ...
        if (currentTtsRequestId === requestId) currentTtsRequestId = "";
        activeTts = false;
        if (!pendingHangup && generation === activeTtsGeneration) {
          assistantSpeaking = false;
          finalResponseCompleted = true;
        }
  ...
  function requestInitialGreeting(reason) {
    ...
    if (voiceProvider === "elevenlabs") {
      transcriptLines.push({
        role: "assistant",
        speaker: "Agent",
        text: greetingState.greetingText,
        ts: new Date().toISOString(),
      });
      queueTranscriptSnapshotSave("initial-greeting", 0);
    ...
    noAudioTimer = setTimeout(() => {
      ...
      if (noProviderAudio) {
        console.warn(
          `[twilio-media-stream] no ${voiceProvider} audio received within 3s`,
          {
            callSid: context.callSid,
            streamSid,
            openaiSessionReady,
  ...
  async function ensureOpenAI() {
    ...
    context.recipientName = firstNonEmpty(
      context.recipientName,
      context.targetName,
    ...
    openaiWs.on("open", () => {
      ...
      logLifecycle("OpenAI session.update sent", {
        ...
        input_audio_format: "g711_ulaw",
        output_audio_format:
          voiceProvider === "elevenlabs" ? "disabled_text_only" : "g711_ulaw",
      });
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "conversation.item.input_audio_transcription.completed") {
        ...
        if (transcript) {
          ...
          queueTranscriptSnapshotSave("caller-transcript-final", 0);
      ...
      if (
        ...
        if (text) {
          ...
          queueTranscriptSnapshotSave("assistant-transcript-final", 0);
      ...
      if (type === "input_audio_buffer.speech_started" && streamSid) {
        ...
        if (VOICE_ALLOW_BARGE_IN && assistantSpeaking) {
          ...
          setTimeout(() => {
            if (
              ...
              console.log("[turn-taking] barge-in detected", {
                ...
                streamSid,
                voiceProvider,
                minSpeechMs: VOICE_BARGE_IN_MIN_SPEECH_MS,
              });
  ...
  twilioWs.on("message", async (raw) => {
    ...
    if (message.event === "stop") {
      ...
      try {
        ...
        console.warn("[transcript] stop-event finalization failed", {
          callSid: context.callSid,
          error: err.message || String(err),
        });
      }
