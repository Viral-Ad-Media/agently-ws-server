...
const CALL_CONTEXT_LOAD_TIMEOUT_MS = Math.max(
  750,
  Number(process.env.CALL_CONTEXT_LOAD_TIMEOUT_MS || 9000),
);
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    // Kick off pre-call intelligence IN PARALLEL with the (slower) knowledge
    // base load below, so the clean tone-aware opening costs ~0 extra wall-clock
    // and never pushes context loading past its timeout / into the fail-safe.
    const precallDirection = String(
      context.direction || agent.direction || "inbound",
    ).toLowerCase();
    const precallPromise =
      precallDirection === "outbound"
        ? buildPrecallOpening({
            direction: "outbound",
            tone: firstNonEmpty(agent.tone, "Professional"),
            agentName: firstNonEmpty(agent.name, context.agentName),
    ...
    // Pre-call intelligence result (started in parallel with KB load above).
    // By now it is almost always resolved, so it adds no perceptible latency.
    let precallOpening = "";
    if (isOutbound) {
      try {
        const precall = await precallPromise;
        ...
        console.warn("[precall-intelligence] skipped:", precallErr.message);
      }
    }

    /*
     * GREETING RESOLUTION — single order, both directions.
     *
     * Outbound previously read `firstNonEmpty(precallOpening, outboundGreeting)`,
     * so pre-call intelligence overrode whatever the tenant had configured.
     * Inbound read agent.greeting correctly, but the value was then shadowed
     * further down by the stream parameters (see the context.greeting merge).
     *
     * resolveOpeningGreeting() applies one order everywhere — configured
     * greeting, then pre-call output, then synthesis — and guarantees the
     * agent introduces itself regardless of which branch won.
     */
    const greeting = voiceBehavior.resolveOpeningGreeting({
      agent,
      organization,
      direction: isOutbound ? "outbound" : "inbound",
      recipientName: context.recipientName || context.targetName || "",
      targetName: context.targetName || context.recipientName || "",
      callPurpose: context.callPurpose || "",
      precomputed: isOutbound
        ? firstNonEmpty(precallOpening, outboundGreeting)
        : firstNonEmpty(defaultInboundGreeting),
      agentName: agent?.name || context.agentName || "",
      organizationName:
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  function safeOpeningGreetingForSpeech(reason = "") {
    ...
    /*
     * Last line of defence. Whatever path produced this greeting — dashboard,
     * pre-call intelligence, synthesiser or DEFAULT_GREETING — the agent must
     * introduce itself before anything else is said, on inbound and outbound
     * alike. If the text already names the agent or the business this is a
     * no-op, so a tenant's own wording is never rewritten.
     */
    greeting = voiceBehavior.ensureAgentIntroduction(greeting, {
      agentName: context.agentName || context.agent?.name || "",
      organizationName:
        context.organizationName ||
        context.agent?.knowledge_base_business_name ||
        context.organization?.name ||
        "",
      direction: isOutboundContext(context) ? "outbound" : "inbound",
    });
        ...
        const [chunkResult, faqResult] = await Promise.all([
          db
            .rpc("search_knowledge_chunks", {
              ...
              p_query: query,
              // Was 6 x 500 — the same window that made web-call agents unable
              // to answer questions whose answers were sitting in their own
              // knowledge base. Chunks routinely carry the fact past character
              // 500, and a page's SEO prose outranks its own data table, so the
              // real answer was neither ranked high enough nor returned in
              // full. Phone calls are the product, so they get the same
              // corrected window.
              p_limit: 20,
              p_max_chars: 1800,
            })
  ...
  async function loadAgentAndPromptWithRealtimeTimeout(currentContext) {
    ...
    try {
      ...
      const failSafeReason = voiceBehavior.humanizeOutboundPurposeForSpeech(
        currentContext.callPurpose || "follow up briefly",
        200,
      );
      ...
      const systemPrompt = outbound
        ? [
            ...
            `Call purpose: ${failSafeReason || "follow up briefly"}.`,
  ...
  async function ensureOpenAIInternal() {
    ...
    /*
     * The loaded greeting now leads.
     *
     * It used to come LAST behind context.openingGreeting and
     * context.greetingMessage, both of which arrive on the stream URL. The API
     * server fills those from its own synthesiser, so the database greeting was
     * resolved correctly here and then discarded on every call — the immediate
     * cause of "the agent never uses my greeting".
     *
     * The API side is fixed too, so in normal operation these now agree. This
     * ordering is the backstop for a cached TwiML document or any direct stream
     * connection that carries stale parameters.
     */
    ...
    // Keep the spoken-greeting resolvers in step with the decision above.
    context.openingGreeting = context.greeting;
    context.greetingMessage = context.greeting;
  ...
  twilioWs.on("close", (code, reason) => {
    ...
    void postBillingUsageEvent({
      organizationId: context.orgId || context.organizationId || null,
      // We run on AWS now. Runtime seconds stay measured because the
      // shared-infrastructure engine apportions the flat Lightsail bill by
      // them, but they are never charged to a tenant as a provider cost.
      provider: "aws",
      service: "runtime",
