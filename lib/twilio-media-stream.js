...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  function createAssistantResponseAfterValidatedCallerTurn(transcript) {
    ...
    if (awaitingHangupConfirmation) return;
    const instruction = [
      "Respond to the caller's latest message naturally and briefly.",
      "Do not repeat the opening greeting; it has already been spoken.",
      "If the caller asks a question, answer from the available business knowledge. If unsure, offer to take a message for the team.",
      `Caller said: ${String(transcript || "").slice(0, 500)}`,
    ].join(" ");
    if (voiceProvider === "elevenlabs") {
  ...
  async function loadAgentAndPromptWithRealtimeTimeout(currentContext) {
    ...
    try {
      ...
      const greeting = outbound
        ...
        : `Hello, thank you for calling ${organizationName}. This is ${agentName}. How can I help you today?`;
