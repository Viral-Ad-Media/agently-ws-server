...
function classifyOpenAIProviderFailure(eventOrError, fallback = "") {
  const err = eventOrError?.error || eventOrError || {};
  const code = String(
    err?.code || err?.type || err?.name || fallback || "",
  ).trim();
  const message = String(err?.message || err?.reason || fallback || "").trim();
  const text = `${code} ${message}`.toLowerCase();
  if (
    /insufficient[_\s-]?quota|quota|exceeded your current quota|billing/.test(
      text,
    )
  ) {
    return {
  ...
  if (
    ...
    return {
      ...
      message: message || "The OpenAI API key is invalid or unauthorized.",
    };
  }
  if (
    ...
    return {
      ...
      message: message || "The configured OpenAI realtime model was not found.",
    };
  }
  if (/rate[_\s-]?limit|too many requests|429/.test(text)) {
    return {
      ...
      message: message || "OpenAI realtime is currently rate limited.",
    };
  }
  if (
    ...
    return {
      provider: "openai",
      code: code || "billing_not_active",
      reason: "billing_not_active",
      message: message || "OpenAI billing is not active.",
    };
  }
  if (/websocket.*closed|closed before audio|no audio|timeout/.test(text)) {
    return {
      provider: "openai",
      code: code || "openai_audio_unavailable",
      reason: /no audio|timeout/.test(text)
        ? "openai_audio_timeout"
        : "openai_websocket_closed_before_audio",
      message: message || "OpenAI realtime closed or produced no audio.",
    };
  }
  return null;
}

function aiProviderFailureMetadata({
  context,
  error,
  counters,
  streamSid,
  ...
  return {
    ai_provider_error: {
      provider: "openai",
      code: error?.code || error?.reason || "provider_runtime_error",
      reason: error?.reason || "provider_runtime_error",
      message: error?.message || "OpenAI realtime failed during the call.",
      occurred_at: new Date().toISOString(),
    },
    call_outcome: "ai_provider_failed",
    message_capture_status: "skipped_ai_provider_failed",
    message_capture_skipped_reason: "ai_provider_failed",
    twilio_call_created: Boolean(context?.callSid),
    openai_session_created: Boolean(openaiSessionCreated),
    openai_audio_received: Number(counters?.openaiAudioDeltasReceived || 0) > 0,
    audio_frames_sent_to_twilio: Number(counters?.audioFramesSentToTwilio || 0),
    openai_audio_deltas_received: Number(
      counters?.openaiAudioDeltasReceived || 0,
    ),
    twilio_media_frames_received: Number(
      counters?.twilioMediaFramesReceived || 0,
    ),
    close_reason: closeReason || "",
    hangup_reason: hangupReason || "ai_provider_failed",
    stream_sid: streamSid || "",
    billing: {
      ai_provider_billable: false,
      twilio_billable: Boolean(context?.callSid || streamSid),
      outcome: "provider_failed_during_call",
    },
  };
}
...
function outboundMessageCaptureEnabled() {
  return (
    String(process.env.OUTBOUND_MESSAGE_CAPTURE_ENABLED || "")
      .trim()
      .toLowerCase() === "true"
  );
}
...
async function loadCallMessageDebug(callSid) {
  ...
  const structuredMessageSaved = voicemailDetected
    ? false
    : Boolean(inbound?.lead_id || inbound?.message);
  const leadSaved = voicemailDetected
    ? false
    : Boolean(canonicalLead?.id || inbound?.lead_id);
...
async function loadCallRecordDebug(callSid) {
  ...
  return {
    ...
    outcome: record?.outcome || metadata.call_outcome || "",
    answered_by: metadata.answered_by || "",
    machine_detection_result: metadata.machine_detection_result || "",
    voicemail_detected: Boolean(metadata.voicemail_detected),
    ai_provider_error: metadata.ai_provider_error || null,
    call_outcome: metadata.call_outcome || record?.outcome || "",
    message_capture_status: metadata.message_capture_status || "",
    twilio_call_created: Boolean(
      metadata.twilio_call_created || record?.twilio_call_sid,
    ),
    openai_session_created: Boolean(metadata.openai_session_created),
    openai_audio_received: Boolean(metadata.openai_audio_received),
    audio_frames_sent_to_twilio: Number(
      metadata.audio_frames_sent_to_twilio || 0,
    ),
    billable: metadata.billing || null,
    transcript_length: transcript.length,
    close_reason: metadata.close_reason || "",
    hangup_reason: metadata.hangup_reason || record?.end_reason || "",
    ...
    recording_storage_path:
      record?.recording_storage_path || metadata?.recording?.storage_path || "",
    recording_duration:
      record?.recording_duration ||
      metadata?.recording?.recording_duration ||
      null,
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    const agentNameForGreeting = firstNonEmpty(agent.name, "your assistant");
    const outboundGreeting = firstNonEmpty(
      agent.outbound_greeting,
      agent.outbound_intro,
      context.callPurpose
        ? `Hello, this is ${agentNameForGreeting} from ${organizationNameForGreeting}. I’m calling about ${context.callPurpose}. Is now a good time?`
        : `Hello, this is ${agentNameForGreeting} from ${organizationNameForGreeting}. I’m calling to follow up. Is now a good time?`,
    );
    const greeting = isOutbound
      ? outboundGreeting
      : firstNonEmpty(agent.greeting, agent.welcome_message, DEFAULT_GREETING);
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function finishMessageCapture() {
    if (aiProviderFailed || context.aiProviderFailed) {
      console.log("[message-capture] skipped due to AI provider failure", {
        callSid: context.callSid,
      });
      return { saved: false, skipped: true, reason: "ai_provider_failed" };
    }
    try {
  ...
  async function requestTwilioCallEnd(reason) {
    if (callEndRequested || callEnded) return;
    if (assistantSpeaking && !finalResponseCompleted) {
      pendingHangup = true;
  ...
  function requestInitialGreeting(reason) {
    ...
    const noAudioTimeoutMs = Math.max(
      5000,
      Number(process.env.OPENAI_REALTIME_FIRST_AUDIO_TIMEOUT_MS || 10000),
    );
    noAudioTimer = setTimeout(() => {
      if (counters.openaiAudioDeltasReceived === 0 && !aiProviderFailed) {
        console.warn(
          "[twilio-media-stream] no OpenAI audio received within safe timeout",
          {
            callSid: context.callSid,
            streamSid,
            openaiSessionReady,
        ...
        void markAiProviderFailed(
          {
            provider: "openai",
            code: "openai_audio_timeout",
            reason: "openai_audio_timeout",
            message:
              "OpenAI realtime produced no audio within the safe timeout.",
          },
          "openai-audio-timeout",
        );
      }
    }, noAudioTimeoutMs);
  }

  async function markAiProviderFailed(
    error,
    closeReason = "ai_provider_failed",
  ) {
    if (aiProviderFailed) return;
    aiProviderFailed = true;
    aiProviderFailureInfo = error || {
      reason: closeReason,
      message: closeReason,
    };
    messageCaptureEnabled = false;
    context.aiProviderFailed = true;
    closingReason = closeReason;
    hangupReason = closeReason;

    console.error(
      "[ai-provider] runtime error provider=openai code=" +
        (aiProviderFailureInfo.code ||
          aiProviderFailureInfo.reason ||
          "unknown"),
      {
        callSid: context.callSid,
        streamSid,
        reason: aiProviderFailureInfo.reason || "",
        message: aiProviderFailureInfo.message || "",
      },
    );
    ...
    const metadata = mergeMetadata(
      existingCallRecord?.metadata,
      aiProviderFailureMetadata({
        context,
        error: aiProviderFailureInfo,
        counters,
        streamSid,
        openaiSessionCreated,
        closeReason,
    ...
    const update = {
      ...
      outcome: "ai_provider_failed",
      transcript: transcriptArray,
      summary: transcriptArray.length
        ? summarizeTranscript(transcriptLines, 900)
        : "AI voice provider failed before a real conversation started.",
      metadata,
      ended_at: new Date().toISOString(),
      end_reason: closeReason,
    };

    if (existingCallRecord?.id || callRecordId) {
      await safeDbWrite("call_records mark ai_provider_failed by id", () =>
        db
          .from("call_records")
          .update(update)
          .eq("id", existingCallRecord?.id || callRecordId)
      ...
      await safeDbWrite("call_records mark ai_provider_failed by sid", () =>
        db
          .from("call_records")
          .update(update)
          .eq("twilio_call_sid", twilioCallSid)
    ...
    console.log("[ai-provider] call marked ai_provider_failed", {
      callSid: context.callSid,
      streamSid,
    });
    console.log("[ai-provider] ending call due to provider failure", {
      callSid: context.callSid,
      streamSid,
      closeReason,
    });
    ...
    try {
      if (openaiWs && openaiWs.readyState === WebSocket.OPEN)
        openaiWs.close(1000, "ai provider failed");
    } catch (_) {}
    try {
      if (context.callSid) await endTwilioCall(context.callSid);
      callEnded = true;
      callEndRequested = true;
    } catch (err) {
      console.error(
        "[ai-provider] failed to end Twilio call after provider failure",
        {
          callSid: context.callSid,
          error: err.message || String(err),
        },
      ...
      try {
        twilioWs.close(1011, "ai provider failed");
      } catch (_) {}
    }
  }
  ...
  async function handleCaptureInboundMessage(rawArgs, callId = "") {
    if (aiProviderFailed) {
      console.log("[message-capture] skipped because AI provider failed", {
        callSid: context.callSid,
        callId,
      });
      return { saved: false, skipped: true, reason: "ai_provider_failed" };
  ...
  async function ensureOpenAI() {
    ...
    if (!apiKey) {
      ...
      await markAiProviderFailed(
        {
          reason: "missing_api_key",
          code: "missing_api_key",
          message: "OPENAI_API_KEY is not configured on Railway.",
        },
        "missing-api-key",
      );
      return;
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "error") {
        ...
        const providerFailure = classifyOpenAIProviderFailure(event, errMsg);
        if (providerFailure) {
          void markAiProviderFailed(providerFailure, providerFailure.reason);
          return;
        }
      ...
      if (type === "conversation.item.input_audio_transcription.completed") {
        ...
        if (transcript) {
          ...
          if (awaitingHangupConfirmation) {
            if (isHangupConfirmationAffirmative(transcript)) {
    ...
    openaiWs.on("error", (err) => {
      ...
      const classified =
        classifyOpenAIProviderFailure(err, err.message) ||
        (counters.openaiAudioDeltasReceived === 0
          ? {
              provider: "openai",
              code: "openai_websocket_error_before_audio",
              reason: "openai_websocket_closed_before_audio",
              message:
                err.message ||
                "OpenAI websocket errored before audio was received.",
            }
          : null);
      if (classified) void markAiProviderFailed(classified, classified.reason);
    });

    openaiWs.on("close", (code, reason) => {
      openaiSocketOpen = false;
      openaiSessionReady = false;
      openaiCloseReason = reason?.toString?.() || String(code || "");
      logLifecycle("OpenAI WS closed", {
        code,
        reason: openaiCloseReason,
        callSid: context.callSid,
        streamSid,
        counters,
      });
      if (
        ...
        void markAiProviderFailed(
          {
            provider: "openai",
            code: "openai_websocket_closed_before_audio",
            reason: "openai_websocket_closed_before_audio",
            message:
              `OpenAI websocket closed before audio was received. close=${code} ${openaiCloseReason}`.trim(),
          },
          "openai-websocket-closed-before-audio",
        );
      }
    });
  }
