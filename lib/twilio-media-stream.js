...
// When caller speech starts while Twilio still has assistant audio buffered,
// clear faster than the normal VAD minimum so the caller can interrupt naturally.
const VOICE_BARGE_IN_CLEAR_MS = intEnv("VOICE_BARGE_IN_CLEAR_MS", 300, 100);
const VOICE_ECHO_GUARD_MS = intEnv("VOICE_ECHO_GUARD_MS", 2200, 500);
...
function mergeStartParameters(context, start) {
  ...
  return {
    ...
    elevenLabsVoiceName: firstNonEmpty(
      ...
      custom.elevenlabsVoiceName,
      context.elevenLabsVoiceName,
    ),
    elevenLabsModel: firstNonEmpty(
      custom.elevenLabsModel,
      custom.elevenlabsModel,
      context.elevenLabsModel,
    ),
...
function hangupConfirmationPrompt(context = {}, reason = "") {
  if (isOutboundContext(context)) {
    return "Before I end the call, is there anything else I can help you with, or should I end the call now?";
  }
  if (/message|callback|lead|complaint/i.test(String(reason || ""))) {
    return "I have your message. Before I end the call, is there anything else you need help with?";
  }
  return "Before I end the call, is there anything else I can help you with?";
}
...
function outboundVoicemailMessage(context = {}) {
  const agentName = firstNonEmpty(context.agentName);
  const organizationName = firstNonEmpty(context.organizationName);
  const identity = voiceBehavior.buildCallerIdentityPhrase({
    agentName,
    organizationName,
  });
  return `Hello, ${identity}. We were calling to follow up. Please call us back when convenient. Thank you.`;
}
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    const organizationNameForGreeting = firstNonEmpty(
      organization?.name,
      context.organizationName,
    );
    const agentNameForGreeting = firstNonEmpty(agent.name, context.agentName);
    ...
    const defaultInboundGreeting = `Hello! You have reached ${organizationNameForGreeting}. This is ${agentNameForGreeting}. What can I help you with today?`;
...
function enhanceSystemPromptForScheduledCall(systemPrompt, context = {}) {
  ...
  const rules = [
    ...
    "- Use the known recipient name when available. Do not ask for their name unless they correct it.",
    ...
    "- Before ending, say you are about to end the call and allow a final response unless they explicitly told you to hang up.",
...
function enhanceSystemPromptForVoice(systemPrompt) {
...
function realtimeAudioPcmuFormat() {
  // Twilio Media Streams sends and expects G.711 u-law at 8kHz.
  // OpenAI Realtime GA represents that as audio/pcmu WITHOUT a rate field.
...
function realtimeTurnDetectionConfig(context = {}) {
  const silenceMs = turnSilenceMsForContext(context);
  return {
    type: "server_vad",
    threshold: VOICE_VAD_THRESHOLD,
    prefix_padding_ms: VOICE_VAD_PREFIX_PADDING_MS,
    silence_duration_ms: silenceMs,
    // We create responses manually after a validated transcription. This avoids
    // the model replying to echo/noise/false short transcripts like "bye".
    create_response: false,
    interrupt_response: true,
  };
}

function realtimeSessionUpdateGa(
  systemPrompt,
  selectedVoice,
  ...
  const session = {
    type: "realtime",
    instructions: enhanceSystemPromptForVoice(systemPrompt),
    ...
    audio: {
      input: {
        format: realtimeAudioPcmuFormat(),
        transcription: realtimeTranscriptionConfig(),
        turn_detection: realtimeTurnDetectionConfig(context),
      },
    },
  };
...
function realtimeSessionUpdate(
  ...
  return realtimeSessionUpdateGa(
    systemPrompt,
    selectedVoice,
...
function buildTextResponse(instructions) {
  return {
...
function buildOpenAiAudioResponse(instructions) {
  return {
    type: "response.create",
    response: {
...
function buildGreetingResponse(greeting) {
  return {
    ...
    response: {
      output_modalities: ["audio"],
      instructions: `Speak exactly this greeting and nothing else. Do not add extra questions, introductions, or helpdesk phrases: ${greeting}`,
    },
  };
}

function buildCurrentGreetingResponse(greeting) {
  return buildGreetingResponse(greeting);
}
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  let twilioPlaybackActive = false;
  let lastAssistantAudioSentAt = 0;
  let lastUserSpeechDurationMs = 0;
  let ttsMarkCounter = 0;
  const pendingTwilioMarks = new Map();
  ...
  function requestHangupConfirmation(reason = "caller-end-intent") {
    ...
    if (voiceProvider === "elevenlabs") {
      ...
      safeSend(openaiWs, buildGreetingResponse(prompt));
    } else {
      logLifecycle(
  ...
  function requestClosingAndHangup(reason, closingMessage = CLOSING_MESSAGE) {
    ...
    if (voiceProvider === "elevenlabs") {
      assistantSpeaking = true;
      finalResponseCompleted = false;
      void speakWithElevenLabs(closingMessage, "closing-message").then(() => {
  ...
  function isAssistantAudioActive() {
    return (
      assistantSpeaking ||
      activeTts ||
      twilioPlaybackActive ||
      pendingTwilioMarks.size > 0 ||
      (lastAssistantAudioSentAt &&
        Date.now() - lastAssistantAudioSentAt < VOICE_ECHO_GUARD_MS)
    );
  }

  function clearAssistantPlayback(reason = "caller-barge-in") {
    activeTtsGeneration += 1;
    lastBargeInAt = Date.now();
    activeTts = false;
    assistantSpeaking = false;
    twilioPlaybackActive = false;
    for (const entry of pendingTwilioMarks.values()) {
      if (entry?.timeout) clearTimeout(entry.timeout);
    }
    pendingTwilioMarks.clear();
    ...
    console.log("[turn-taking] assistant playback cleared", {
      callSid: context.callSid,
      streamSid,
      reason,
      voiceProvider,
  ...
  function sendTwilioPlaybackMark(label, requestId, audioBytes = 0) {
    if (!streamSid || callEndRequested || callEnded) return "";
    const name = `${label || "tts"}:${requestId || ++ttsMarkCounter}`.slice(
      0,
      120,
    );
    const timeoutMs = Math.max(
      2500,
      Math.min(30000, Math.ceil((Number(audioBytes) || 0) / 8) + 2500),
    );
    twilioPlaybackActive = true;
    assistantSpeaking = true;
    const timeout = setTimeout(() => {
      if (!pendingTwilioMarks.has(name)) return;
      pendingTwilioMarks.delete(name);
      if (!activeTts && pendingTwilioMarks.size === 0) {
        twilioPlaybackActive = false;
        if (
      ...
      console.warn("[turn-taking] Twilio playback mark timeout", {
        callSid: context.callSid,
        streamSid,
        name,
        timeoutMs,
    ...
    pendingTwilioMarks.set(name, { label, requestId, timeout, audioBytes });
    safeSend(twilioWs, { event: "mark", streamSid, mark: { name } });
    console.log("[turn-taking] Twilio playback mark sent", {
      callSid: context.callSid,
      streamSid,
      name,
      audioBytes,
  ...
  function handleTwilioPlaybackMark(message) {
    const name = String(message?.mark?.name || "").trim();
    if (!name || !pendingTwilioMarks.has(name)) return;
    const entry = pendingTwilioMarks.get(name);
    if (entry?.timeout) clearTimeout(entry.timeout);
    pendingTwilioMarks.delete(name);
    if (!activeTts && pendingTwilioMarks.size === 0) {
      twilioPlaybackActive = false;
    ...
    console.log("[turn-taking] Twilio playback mark received", {
      callSid: context.callSid,
      streamSid,
      name,
      pendingMarks: pendingTwilioMarks.size,
    });
  }
    ...
    const queuedJob = async () => {
      ...
      try {
        for (const chunk of chunks) {
          ...
          if (ttsFramesSentForRequest > 0) {
            sendTwilioPlaybackMark(label, requestId, ttsAudioBytes);
          }
  ...
  function isLikelyAsrHallucination(transcript) {
    ...
    const commonHallucinations = [
      /^y lo describire en el libro$/i,
      /^y lo describir[eé] en el libro$/i,
      /^to watch all of the songs from me$/i,
      /^thank you for watching$/i,
      /^thanks for watching$/i,
      /^subtitles by.*$/i,
      /^music$/i,
    ];
    if (commonHallucinations.some((pattern) => pattern.test(normalized))) {
      return true;
    }

    const englishCall = /^(en|eng|english)$/i.test(
      String(context.sessionLanguage || context.language || "English").trim(),
    );
    const looksMostlySpanish =
      /\b(el|la|los|las|una|uno|libro|describire|describiré|gracias|adios|hola)\b/i.test(
        normalized,
      );
    if (englishCall && looksMostlySpanish && !/[?]/.test(normalized)) {
      return true;
    }

    // If the assistant is still audible and ASR returns a phrase that is not a
    // direct answer to the call, prefer asking again over letting the model
    // build a full business response on a hallucinated transcript.
    const recentlyAssistantAudio =
      isAssistantAudioActive() ||
      (lastAssistantAudioSentAt &&
        Date.now() - lastAssistantAudioSentAt < 3500);
    if (
      recentlyAssistantAudio &&
      /\b(virtual assistant|online|physical place|not tied|not making unsolicited|thanks for watching|songs from me)\b/i.test(
        normalized,
      )
    ) {
      return true;
  ...
  function isLikelyLowValueCallerTranscript(transcript) {
    const normalized = normalizeSpeechText(transcript).toLowerCase();
    if (!normalized) return true;
    if (isLikelyAsrHallucination(transcript)) return true;
    if (/^(you|uh|um|hmm|mm|noise)$/.test(normalized)) return true;
    const shortGoodbye =
      /^(bye|goodbye|bye bye|thank you bye|thanks bye|thank you goodbye|thanks goodbye)$/.test(
        normalized,
      );
    if (shortGoodbye) {
      const recentlyAssistantAudio =
        isAssistantAudioActive() ||
        (lastAssistantAudioSentAt &&
          Date.now() - lastAssistantAudioSentAt < 5000);
      const notAfterPermissionPrompt =
        !lastAssistantAskedAnythingElse &&
        !lastAssistantAskedEndPermission &&
        !awaitingHangupConfirmation;
      if (
        notAfterPermissionPrompt &&
        (recentlyAssistantAudio || lastUserSpeechDurationMs < 1800)
      ) {
        return true;
      }
    }
  ...
  function buildGroundedResponseInstruction(transcript) {
    const agentName = firstNonEmpty(context.agentName);
    ...
    const agentLabel = voiceBehavior.cleanAgentNameForSpeech(agentName);
    const organizationLabel =
      voiceBehavior.cleanOrganizationNameForSpeech(organizationName);
    const identityPhrase = voiceBehavior.buildCallerIdentityPhrase({
      agentName: agentLabel,
      organizationName: organizationLabel,
    });
    const purpose = sanitizeOutboundPurposeText(
      context.callPurpose || "a customer follow-up",
      280,
    );
    const callerText = String(transcript || "").slice(0, 700);

    const identity = isOutboundContext(context)
      ? [
          `You are the same outbound phone agent that started this call. Your spoken identity phrase is: "${identityPhrase}."`,
          organizationLabel
            ? `You are calling on behalf of ${organizationLabel}.`
            : "You are calling on behalf of the tenant/business configured for this call.",
          agentLabel
            ? `Your configured agent name is ${agentLabel}.`
            : "No safe agent name was provided, so do not invent one.",
          "Never switch to a generic assistant identity after interruption.",
          "Never deny that this outbound call was made by the business/tenant that scheduled it.",
        ].join(" ")
      : [
          agentLabel
            ? `You are ${agentLabel}, the inbound phone agent for this business.`
            : "You are the assigned inbound phone agent for this business.",
          organizationLabel
            ? `The caller contacted ${organizationLabel}.`
            : "The caller contacted the configured business/tenant.",
        ].join(" ");

    const purposeRule = isOutboundContext(context)
      ? [
          `The reason for this call is: ${purpose || "a customer follow-up"}.`,
          organizationLabel
            ? `If asked why you are calling or where the number came from, calmly explain that you are calling on behalf of ${organizationLabel} for this purpose.`
            : "If asked why you are calling, calmly explain the configured call purpose without inventing business details.",
          "Keep it brief and offer to take a message, schedule a callback, honor opt-out, or end the call if preferred.",
        ].join(" ")
      : "The caller contacted the business; help them using the business context.";

    return [
      identity,
      purposeRule,
      "Respond to the caller's latest message naturally and briefly in one or two short sentences.",
      "Do not repeat the opening greeting; it has already been spoken.",
      "Use the business knowledge base and call purpose. If the transcript seems garbled, ask the caller to repeat instead of guessing.",
      "Do not say: 'I am a virtual assistant here to help', 'I am not making unsolicited calls', 'I am not tied to any physical place', or 'How can I assist you today?' in outbound calls.",
      `Caller transcript, which may contain ASR errors: ${callerText}`,
    ].join(" ");
  }

  function createAssistantResponseAfterValidatedCallerTurn(transcript) {
    ...
    if (
      ...
      return;
    if (awaitingHangupConfirmation) return;
    const instruction = buildGroundedResponseInstruction(transcript);
    if (voiceProvider === "elevenlabs") {
      safeSend(openaiWs, buildTextResponse(instruction));
    } else {
      safeSend(openaiWs, buildOpenAiAudioResponse(instruction));
    }
  }
  ...
  function applyEarlyVoiceHintsFromContext() {
    const hintedProvider = normalizeProvider(
      context.voiceProviderHint || context.voiceProviderOverride || "",
      "",
    );
    if (hintedProvider) voiceProvider = hintedProvider;
    if (context.openAiVoice) context.sessionVoice = context.openAiVoice;
    if (context.greetingMessage && !context.greeting) {
      context.greeting = context.greetingMessage;
    }
    if (context.elevenLabsVoiceId && !elevenLabsVoice) {
      elevenLabsVoice = {
        voiceId: context.elevenLabsVoiceId,
        name: context.elevenLabsVoiceName || context.voiceProfile || "",
        modelId: context.elevenLabsModel || elevenLabsConfig().defaultModel,
      };
    }
  }

  function maybeStartEarlyOutboundGreeting(reason) {
    const enabled =
      String(process.env.OUTBOUND_EARLY_GREETING_ENABLED || "true")
        .trim()
        .toLowerCase() !== "false";
    if (!enabled || !isOutboundContext(context)) return;
    applyEarlyVoiceHintsFromContext();
    if (!context.greeting && !context.greetingMessage) return;
    // ElevenLabs can speak immediately from Stream parameters while the full
    // Supabase knowledge context loads. OpenAI voice still needs the Realtime
    // socket, so it starts after ensureOpenAI() completes.
  ...
  function requestInitialGreeting(reason) {
    ...
    if (voiceProvider === "elevenlabs") {
      ...
      safeSend(openaiWs, buildGreetingResponse(greetingState.greetingText));
  ...
  async function loadAgentAndPromptWithRealtimeTimeout(currentContext) {
    ...
    try {
      ...
      const agentName = firstNonEmpty(currentContext.agentName);
      const organizationName = firstNonEmpty(
      ...
      const systemPrompt = outbound
        ? [
            `You are ${agentName}, a warm and respectful outbound phone representative for ${organizationName}.`,
        ...
        : [
            `You are ${agentName}, a warm phone receptionist for ${organizationName}.`,
      ...
      const hintedVoiceProvider = normalizeProvider(
        currentContext.voiceProviderHint ||
          currentContext.voiceProviderOverride ||
          process.env.VOICE_PROVIDER_DEFAULT,
        "openai",
      );
      const hintedElevenLabsVoice = currentContext.elevenLabsVoiceId
        ? {
            voiceId: currentContext.elevenLabsVoiceId,
            name:
              currentContext.elevenLabsVoiceName ||
              currentContext.voiceProfile ||
              "",
            modelId:
              currentContext.elevenLabsModel || elevenLabsConfig().defaultModel,
          }
        : null;
      return {
        ...
        systemPrompt,
        greeting: currentContext.greetingMessage || greeting,
        selectedVoice: currentContext.openAiVoice || selectedVoice,
        openAiVoice: currentContext.openAiVoice || selectedVoice,
        voiceProvider: hintedVoiceProvider,
        fallbackProvider: process.env.VOICE_PROVIDER_FALLBACK || "openai",
        elevenLabsVoice: hintedElevenLabsVoice,
        voiceSettings: {},
        language: "English",
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "error") {
        ...
        if (
          ...
          /input_audio_format|output_audio_format|unknown parameter|invalid|invalid_value|g711_ulaw|session\.audio\.input\.format|session\.type|beta api|realtime beta api|session object/i.test(
            errMsg,
          )
        ) {
      ...
      if (type === "conversation.item.input_audio_transcription.completed") {
        ...
        if (transcript) {
          if (isLikelyLowValueCallerTranscript(transcript)) {
            console.warn(
              "[transcript] ignored likely ASR echo/low-value transcript",
              {
                callSid: context.callSid,
                streamSid,
                text: transcript,
                lastUserSpeechDurationMs,
                assistantAudioActive: isAssistantAudioActive(),
              },
            );
            suppressAssistantResponseUntil = Date.now() + 1200;
            return;
          }
          ...
          if (awaitingHangupConfirmation) {
            ...
            createAssistantResponseAfterValidatedCallerTurn(transcript);
          }
        }
        return;
      }
      ...
      if (type === "input_audio_buffer.speech_started" && streamSid) {
        ...
        // Delay barge-in clearing until speech persists long enough to avoid
        // killing ElevenLabs output because of breath/noise/clicks.
        if (VOICE_ALLOW_BARGE_IN && isAssistantAudioActive()) {
          const speechStartToken = userSpeechStartedAt;
          const clearDelayMs =
            twilioPlaybackActive || pendingTwilioMarks.size > 0
              ? Math.min(VOICE_BARGE_IN_MIN_SPEECH_MS, VOICE_BARGE_IN_CLEAR_MS)
              : VOICE_BARGE_IN_MIN_SPEECH_MS;
          setTimeout(() => {
            if (
              ...
              clearAssistantPlayback("meaningful-caller-speech");
              console.log("[turn-taking] barge-in detected", {
                callSid: context.callSid,
                streamSid,
                voiceProvider,
                minSpeechMs: clearDelayMs,
              });
              logLifecycle("Twilio clear sent after meaningful caller speech", {
                callSid: context.callSid,
                streamSid,
                closingState,
              });
      ...
      if (type === "input_audio_buffer.speech_stopped" && streamSid) {
        ...
        const speechDurationMs = userSpeechStartedAt
          ...
          : 0;
        lastUserSpeechDurationMs = speechDurationMs;
  ...
  twilioWs.on("message", async (raw) => {
    ...
    if (message.event === "start") {
      ...
      maybeStartEarlyOutboundGreeting("twilio-start-early-params");
      try {
        await ensureOpenAI();
      } catch (err) {
    ...
    if (message.event === "mark") {
      handleTwilioPlaybackMark(message);
      return;
    }
