...
const VOICE_TURN_SILENCE_MS = intEnv("VOICE_TURN_SILENCE_MS", 1000, 300);
const VOICE_MIN_USER_SPEECH_MS = intEnv("VOICE_MIN_USER_SPEECH_MS", 500, 100);
const VOICE_RESPONSE_DEBOUNCE_MS = intEnv("VOICE_RESPONSE_DEBOUNCE_MS", 250, 0);
const VOICE_TTS_CHUNK_PAUSE_MS = intEnv("VOICE_TTS_CHUNK_PAUSE_MS", 120, 0);
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    const configuredVoiceProvider = normalizeProvider(
      firstNonEmpty(agent.voice_provider, process.env.VOICE_PROVIDER_DEFAULT),
      "openai",
    );
...
function prepareAssistantTextForTts(text) {
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function speakWithElevenLabs(text, label = "assistant") {
    const clean = prepareAssistantTextForTts(text);
    if (!clean) return;
    ...
    const chunks = splitTextForSpeech(clean, config.maxCharsPerChunk);
    ...
    ttsQueue = ttsQueue.then(async () => {
      ...
      for (const chunk of chunks) {
        ...
        try {
          ...
          // ElevenLabs ulaw_8000 returns raw G.711 μ-law bytes. Twilio Media
          // Streams expects that payload base64-encoded inside a media frame.
  ...
  function requestInitialGreeting(reason) {
    ...
    const greeting = context.greeting || DEFAULT_GREETING;
    if (voiceProvider === "elevenlabs") {
      void speakWithElevenLabs(greeting, "initial-greeting");
    } else {
      safeSend(openaiWs, buildGreetingResponse(greeting));
    }
  ...
  async function ensureOpenAI() {
    ...
    openaiWs.on("open", () => {
      ...
      safeSend(
        openaiWs,
        voiceProvider === "elevenlabs"
          ? realtimeSessionUpdateTextOnly(systemPrompt, messageCaptureEnabled)
          : realtimeSessionUpdate(
              systemPrompt,
              selectedVoice,
              messageCaptureEnabled,
    ...
    openaiWs.on("message", (data, isBinary) => {
        ...
        if (
          ...
          if (voiceProvider === "elevenlabs") {
            void speakWithElevenLabs(
              context.greeting || DEFAULT_GREETING,
              "session-fallback-greeting",
            );
          } else {
            safeSend(
              openaiWs,
              buildCurrentGreetingResponse(
                context.greeting || DEFAULT_GREETING,
              ),
            );
          }
      ...
      if (
        ...
        if (text) {
          const preparedAssistantText = prepareAssistantTextForTts(text);
          transcriptLines.push({
            role: "assistant",
