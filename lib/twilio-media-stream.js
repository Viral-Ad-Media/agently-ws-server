...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  function markActivity(reason) {
    ...
    idleTimer = setTimeout(() => {
      ...
      void requestClosingAndHangup(
        "idle-timeout",
        "I'm going to end the call now. Please call us back anytime. Goodbye.",
      );
    }, IDLE_TIMEOUT_MS);
    resetNoSpeechTimer("caller-activity");
  }
  ...
  function startNoSpeechTimer(reason = "after-greeting") {
    ...
    noSpeechTimer = setTimeout(() => {
      if (callEndRequested || callEnded || callerSpeechDetected) return;
      noSpeechTimeoutFired = true;
      console.log("[no-speech] timeout reached seconds=15", {
        callSid: context.callSid,
        streamSid,
        seconds: Math.round(NO_SPEECH_TIMEOUT_MS / 1000),
      });
