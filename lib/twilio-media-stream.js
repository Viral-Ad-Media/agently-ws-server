  ...
  try {
    ...
    if (context.orgId) {
      const { data: org, error: orgError } = await db
        .from("organizations")
        .select(
          "id, name, industry, website, location, timezone, phone_number, active_voice_agent_id",
        )
        .eq("id", context.orgId)
        .maybeSingle();
      if (orgError) {
    ...
    if (!agent && context.orgId && organization?.active_voice_agent_id) {
      const { data, error } = await db
        .from("voice_agents")
        .select("*")
      ...
      if (error) {
        console.warn(
          "[twilio-media-stream] fallback active agent lookup warning:",
          error.message,
        );
      }
      agent = data || null;
    ...
    try {
      voiceContext = await loadVoiceContext(
        db,
        context.orgId || agent.organization_id,
        agent,
    ...
    logLifecycle("agent loaded", {
      ...
      organizationId: context.orgId || agent.organization_id,
      organizationName: organization?.name || "",
      language,
      voiceProfile,
...
async function loadTwilioAgentContextForDebug({ orgId, agentId }) {
  ...
  return {
    ...
    organizationName: loaded.organization?.name || "",
    ...
    counts: loaded.voiceContext?.stats || {
      faqs: 0,
      knowledgeChunks: 0,
      scrapedPages: 0,
      linkedChatbots: 0,
      finalPromptChars: loaded.systemPrompt.length,
    },
    promptPreview: String(loaded.systemPrompt || "").slice(0, 500),
  };
}
