...
function selectedKnowledgeBaseIdsForContext(context = {}) {
  const ids = [
    context.selectedKnowledgeBaseId,
    context.knowledgeBaseId,
    context.knowledge_base_id,
    context.agent?.knowledge_base_id,
    context.selectedKnowledgeBase?.id,
    context.knowledgeBase?.id,
    context.voiceContext?.stats?.selectedKnowledgeBaseId,
    context.voiceContext?.debug?.knowledgeBase?.id,
  ]
...
function hasSelectedKnowledgeBase(context = {}) {
  return selectedKnowledgeBaseIdsForContext(context).length > 0;
}
...
function realtimeToolDefinitions(enableMessageCapture = true, context = {}) {
  const tools = [];
  if (enableMessageCapture) tools.push(captureToolDefinition());
  if (hasSelectedKnowledgeBase(context))
    tools.push(knowledgeSearchToolDefinition());
  return tools;
}
...
function enhanceSystemPromptForVoice(systemPrompt, context = {}) {
  ...
  if (hasSelectedKnowledgeBase(context)) {
    rules.push(
      "Live Knowledge Search rule:",
      "- You have a search_business_knowledge tool for the selected Knowledge Base only. Use it before answering specific product, service, price, policy, booking, ingredient, availability, shipping, URL, or FAQ questions when the answer is not already obvious from the loaded prompt.",
      "- Never tell callers you are checking a knowledge base, searching a database, or using a tool. Search silently, then answer naturally from the returned results.",
      "- If live search returns no useful result, do not invent. Offer to take a message or callback request and save it with capture_inbound_message when appropriate.",
    );
  }
...
function realtimeSessionUpdateCurrent(
  ...
  return {
    ...
    session: {
      ...
      tools: realtimeToolDefinitions(enableMessageCapture, context),
      tool_choice: realtimeToolDefinitions(enableMessageCapture, context).length
        ? "auto"
        : "none",
      audio: {
...
async function insertBillingUsageEventDirect(rawEvent) {
  ...
  try {
    ...
    return { direct: true, id: insertedId || null };
  } catch (err) {
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function speakWithElevenLabs(text, label = "assistant") {
    ...
    if (
      ...
      !["closing-message"].includes(label)
    ) {
      console.log(
  ...
  async function handleSearchBusinessKnowledge(rawArgs, callId = "") {
    ...
    const query = cleanCapturedMessageText(args.query || "", 500);
    const knowledgeBaseIds = selectedKnowledgeBaseIdsForContext(context);
    ...
    if (!query) {
      result.message =
        "No search query was provided. Ask one short clarifying question or offer to take a message.";
        ...
        result = {
          ...
          message:
            (faqs || []).length || (chunks || []).length
              ...
              : "No useful selected-Knowledge-Base result found. Do not invent; offer to take a message or callback request.",
        };
      } catch (err) {
    ...
    console.log("[knowledge-search] tool result", {
      callSid: context.callSid,
      streamSid,
      callId,
      query,
  ...
  async function ensureOpenAIInternal() {
    ...
    context.voiceContext = voiceContext || context.voiceContext || null;
    context.selectedKnowledgeBase =
      voiceContext?.selectedKnowledgeBase ||
      context.selectedKnowledgeBase ||
      null;
    context.knowledgeBase =
      voiceContext?.selectedKnowledgeBase || context.knowledgeBase || null;
    context.selectedKnowledgeBaseId = firstNonEmpty(
      voiceContext?.stats?.selectedKnowledgeBaseId,
      voiceContext?.debug?.knowledgeBase?.id,
      voiceContext?.selectedKnowledgeBase?.id,
      agent?.knowledge_base_id,
      context.selectedKnowledgeBaseId,
      context.knowledgeBaseId,
    );
    context.knowledgeBaseId =
      context.selectedKnowledgeBaseId || context.knowledgeBaseId || "";
    ...
    openaiWs.on("message", async (data, isBinary) => {
      ...
      if (
        ...
        if (name === "search_business_knowledge") {
          void handleSearchBusinessKnowledge(args, callId);
          return;
        }
      }
