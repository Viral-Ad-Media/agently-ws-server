...
function sanitizeOutboundPurposeText(text, maxChars = 150) {
  ...
  value = firstSentence
    .replace(/^(?:i\s+am|i'm|we\s+are|we're)\s+(?:calling|reaching\s+out)\s+(?:because|to|about|regarding|for)\s+/i, "")
    .replace(/^this\s+call\s+is\s+(?:for|about|regarding)\s+/i, "")
    .replace(/^for\s+the\s+purpose\s+of\s+/i, "")
    .replace(/^(?:the\s+)?purpose\s+(?:of\s+this\s+call\s+)?(?:is\s+|was\s+|of\s+|to\s+)?/i, "")
...
function outboundGreetingReasonClause(text) {
  ...
  if (/^(check|confirm|schedule|reschedule|follow|ask|remind|notify|inform|discuss|update|invite|verify|collect|share|see|reach|help|offer|introduce|review|explain)\b/i.test(purpose)) {
...
function repairOutboundPurposePhrasing(text) {
  return String(text || "")
    .replace(/\b(I\s*(?:am|'m)\s+)?calling\s+because\s+to\b/gi, "I'm calling to")
    .replace(/\b(I\s*(?:am|'m)\s+)?calling\s+because\s+for\s+the\s+purpose\s+of\b/gi, "I'm calling to")
    .replace(/\b(I\s*(?:am|'m)\s+)?calling\s+to\s+for\s+the\s+purpose\s+of\b/gi, "I'm calling to")
...
function cleanCapturedMessageText(text, maxChars = 1800) {
  let value = String(text || "")
    .replace(/\r/g, "\n")
    .split("\n")
    .map((line) => line.replace(/^\s*(user|caller|recipient|assistant|agent)\s*:\s*/i, "").trim())
    .filter(Boolean)
    .join(" ")
    .replace(/^"|"$/g, "")
    .replace(/^(?:the\s+)?message\s+(?:is|was)\s*[:,-]?\s*/i, "")
    .replace(/^please\s+(?:tell|let)\s+(?:them|the\s+team|someone)\s+(?:know\s+)?(?:that\s+)?/i, "")
    .replace(/\s+/g, " ")
    .trim();
  if (!value) return "";
  if (value.length > maxChars) value = value.slice(0, maxChars - 1).replace(/[\s,;:.-]+$/, "") + "...";
  return value;
}
...
async function loadCallMessageDebug(callSid) {
  ...
  const capturedMessage =
    inbound ||
    outbound ||
    (metadata?.message_captured || metadata?.captured_message || metadata?.message
      ? {
          lead_id: null,
          message:
            metadata?.message_captured || metadata?.captured_message || metadata?.message,
  ...
  const canonicalLead = voicemailDetected
    ? null
    : metadataLead || chooseCanonicalLead(leads, capturedMessage?.lead_id || null);
...
function maxAssistantSentencesForContext(context = {}) {
  if (isScheduledOutboundContext(context)) return SCHEDULED_CALL_MAX_ASSISTANT_SENTENCE_COUNT;
  if (isOutboundContext(context)) return OUTBOUND_CALL_MAX_ASSISTANT_SENTENCE_COUNT;
  return 0;
}

function shouldDisableOutboundMonologue(context = {}) {
  if (isScheduledOutboundContext(context)) return SCHEDULED_CALL_DISABLE_MONOLOGUE;
...
function prepareAssistantTextForTts(text, context = {}) {
  ...
  if (isOutboundContext(context)) output = repairOutboundPurposePhrasing(output);
  if (VOICE_DISABLE_FILLER_ACKS) {
    output = output
      .replace(
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function speakWithElevenLabs(text, label = "assistant") {
    ...
    const queuedJob = async () => {
      ...
      if (label === "assistant-response" && lastBargeInAt && requestCreatedAt < lastBargeInAt) {
        console.log("[turn-taking] skipped stale assistant response after barge-in", {
          callSid: context.callSid,
          streamSid,
          label,
          requestId,
      ...
      if (label === "assistant-response" && userSpeechActive) {
        console.log("[turn-taking] skipped assistant response because caller is speaking", {
          callSid: context.callSid,
          streamSid,
          label,
          requestId,
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "response.created") {
        if (
          ...
          assistantTextBuffer = "";
          console.log("[turn-taking] cancelled OpenAI response after short caller audio", {
            callSid: context.callSid,
            streamSid,
          });
          return;
