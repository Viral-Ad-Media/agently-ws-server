...
function conciseOutboundPurposeForGreeting(text, maxChars = 140) {
  ...
  const value =
    firstSentence.length > maxChars
      ? firstSentence.slice(0, maxChars - 1).replace(/[\s,;:.-]+$/, "") + "…"
      : firstSentence;
  return value;
}
...
async function persistInboundCallMessage({
  ...
  if (direction === "outbound" && !context.leadId) {
    ...
    await createTenantNotification({
      ...
      metadata: { outbound_call_message: outboundMessage },
    });
...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    const concisePurpose = conciseOutboundPurposeForGreeting(
    ...
    const outboundGreeting = firstNonEmpty(
      ...
      recipientNameForGreeting && concisePurpose
        ? `Hello ${recipientNameForGreeting}, this is ${agentNameForGreeting} from ${organizationNameForGreeting}. I’m calling because ${concisePurpose}. Is now a good time?`
        : recipientNameForGreeting
          ? `Hello ${recipientNameForGreeting}, this is ${agentNameForGreeting} from ${organizationNameForGreeting}. I’m calling to follow up. Is now a good time?`
          : concisePurpose
            ? `Hello, this is ${agentNameForGreeting} from ${organizationNameForGreeting}. I’m calling because ${concisePurpose}. Is now a good time?`
...
function enhanceSystemPromptForScheduledCall(systemPrompt, context = {}) {
  if (!isScheduledOutboundContext(context)) return systemPrompt;
  const rules = [
    "Scheduled outbound call rules:",
    "- You are making a scheduled outbound call. Keep responses brief.",
    "- Ask one question, then wait for the recipient to respond.",
    "- Do not monologue. Do not continue speaking unless the recipient responds.",
    "- Do not repeat the greeting, agent name, business name, or call purpose.",
    "- If the recipient interrupts, stop speaking and listen.",
    `- Keep each assistant turn to at most ${SCHEDULED_CALL_MAX_ASSISTANT_SENTENCE_COUNT} short sentence(s), unless the recipient asks for detail.`,
  ];
  if (context.callPurpose) rules.push(`- Call purpose: ${context.callPurpose}`);
  if (context.customInstructions)
    rules.push(
      `- Additional scheduled-call instruction: ${context.customInstructions}`,
    );
...
function clipScheduledAssistantText(text, context = {}) {
  if (!isScheduledOutboundContext(context) || !SCHEDULED_CALL_DISABLE_MONOLOGUE)
    return text;
  const parts = String(text || "")
    .split(/(?<=[.!?])\s+/)
    .map((part) => part.trim())
    .filter(Boolean);
  if (parts.length <= SCHEDULED_CALL_MAX_ASSISTANT_SENTENCE_COUNT) return text;
  const clipped = parts
    .slice(0, SCHEDULED_CALL_MAX_ASSISTANT_SENTENCE_COUNT)
    .join(" ");
  console.log("[voice-quality] assistant response clipped to concise mode", {
    callSid: context.callSid || "",
    originalSentences: parts.length,
    keptSentences: SCHEDULED_CALL_MAX_ASSISTANT_SENTENCE_COUNT,
  });
  return clipped;
}
...
async function handleTwilioMediaStreamWS(twilioWs, req) {
  ...
  async function speakWithElevenLabs(text, label = "assistant") {
    ...
    const queuedJob = async () => {
      ...
      if (VOICE_RESPONSE_DEBOUNCE_MS > 0 && label === "assistant-response") {
        console.log("[turn-taking] response delayed", {
          callSid: context.callSid,
          streamSid,
          ms: VOICE_RESPONSE_DEBOUNCE_MS,
        });
        await sleep(VOICE_RESPONSE_DEBOUNCE_MS);
      }
  ...
  async function ensureOpenAI() {
    ...
    console.log("[turn-taking] config", {
      ...
      minUserSpeechMs: VOICE_MIN_USER_SPEECH_MS,
      responseDebounceMs: VOICE_RESPONSE_DEBOUNCE_MS,
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "input_audio_buffer.speech_stopped" && streamSid) {
        ...
        if (
          speechDurationMs > 0 &&
          speechDurationMs < VOICE_MIN_USER_SPEECH_MS
        ) {
          suppressAssistantResponseUntil =
            Date.now() + Math.max(1500, VOICE_TURN_SILENCE_MS);
          console.log("[turn-taking] ignored short partial", {
            callSid: context.callSid,
            streamSid,
            speechDurationMs,
