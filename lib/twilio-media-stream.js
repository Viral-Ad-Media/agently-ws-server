...
function dynamicIdentityRules(context = {}) {
  ...
  const rules = ["Dynamic SaaS agent identity rules:"];
  if (agentName)
...
function openAiRealtimeTranscriptionConfig() {
  const config = {
    model: OPENAI_REALTIME_TRANSCRIPTION_MODEL,
  };
  if (OPENAI_REALTIME_TRANSCRIPTION_LANGUAGE) {
    config.language = OPENAI_REALTIME_TRANSCRIPTION_LANGUAGE;
  }
...
function buildGreetingResponse(greeting) {
  return buildCurrentGreetingResponse(greeting);
}
