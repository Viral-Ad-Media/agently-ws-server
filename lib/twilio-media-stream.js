...
function useCurrentRealtimeSessionShape() {
  const explicit = String(process.env.OPENAI_REALTIME_SESSION_SHAPE || "")
    .trim()
    .toLowerCase();
  if (["legacy", "classic", "audio_format"].includes(explicit)) return false;
  if (["current", "ga", "realtime"].includes(explicit)) return true;
  const model = String(
    process.env.OPENAI_REALTIME_MODEL || "gpt-realtime",
  ).toLowerCase();
  const url = String(OPENAI_REALTIME_URL || "").toLowerCase();
  return model.includes("gpt-realtime") || url.includes("gpt-realtime");
}
...
function buildRealtimeSessionUpdate(
  ...
  if (useCurrentRealtimeSessionShape()) {
    return textOnly
  ...
  return textOnly
    ? realtimeSessionUpdateTextOnly(systemPrompt, enableMessageCapture, context)
...
function buildFallbackRealtimeSessionUpdate(
  systemPrompt,
  selectedVoice,
  enableMessageCapture = true,
  context = {},
...
function buildRealtimeResponse(response) {
  const next = { ...(response || {}) };
  if (useCurrentRealtimeSessionShape()) {
    if (next.modalities && !next.output_modalities) {
      next.output_modalities = next.modalities;
      delete next.modalities;
    }
  } else if (next.output_modalities && !next.modalities) {
    next.modalities = next.output_modalities;
    delete next.output_modalities;
  }
  return { type: "response.create", response: next };
}
  ...
  async function handleCaptureInboundMessage(rawArgs, callId = "") {
    ...
    if (openaiWs && openaiWs.readyState === WebSocket.OPEN) {
      ...
      if (voiceProvider === "elevenlabs") {
        ...
        safeSend(openaiWs, buildOpenAiAudioResponse(outputText));
      }
    }
    return result;
  }
    ...
    openaiWs.on("open", () => {
      ...
      safeSend(
        ...
        buildRealtimeSessionUpdate(
          ...
          messageCaptureEnabled,
          context,
          { textOnly: voiceProvider === "elevenlabs" },
        ),
      );
      ...
      logLifecycle("OpenAI session.update sent", {
        ...
        sessionShape: useCurrentRealtimeSessionShape()
          ? "current-ga"
          : "legacy",
      });
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "error") {
        ...
        if (
          ...
          /input_audio_format|output_audio_format|unknown parameter|invalid|beta api|realtime beta api|session object|missing required parameter|session\.type/i.test(
            errMsg,
          )
        ) {
          ...
          safeSend(
            openaiWs,
            buildFallbackRealtimeSessionUpdate(
              scheduledSystemPrompt,
              selectedVoice,
              messageCaptureEnabled,
              context,
