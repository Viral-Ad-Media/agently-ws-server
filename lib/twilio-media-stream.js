...
async function loadAgentAndPrompt(context) {
  ...
  try {
    ...
    try {
      const contextDirection = context.direction || agent.direction || "inbound";
      const contextQuery = String(contextDirection).toLowerCase() === "outbound"
        ? [
            "outbound phone call",
            context.callPurpose || "",
            context.customInstructions || "",
      ...
      if (
        ...
        if (!openaiWs) {
          void ensureOpenAI().catch((err) => {
            console.error("[twilio-media-stream] ensureOpenAI failed on media", {
              callSid: context.callSid,
              streamSid,
              error: err.message || String(err),
            });
