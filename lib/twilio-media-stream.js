...
function hasEndIntent(text, options = {}) {
  ...
  // Be conservative. In phone calls, phrases like "thank you", "that's it", or
  // "no more" often mean the caller finished one answer/message, not the entire
  // conversation. Only end the call on explicit goodbye/hangup language, or on
  // a combined thanks+bye style phrase.
  const explicitClose =
    /(bye|goodbye|good bye|talk to you later|end the call|hang up|disconnect|you can hang up|please hang up)/.test(
      t,
    ) ||
    /(thank you|thanks).*(bye|goodbye|good bye|have a nice day|wonderful day)/.test(
      t,
    ) ||
    /(bye|goodbye|good bye).*(thank you|thanks|have a nice day|wonderful day)/.test(
      t,
    );

  if (explicitClose) return true;

  // Optional strict follow-up closure. This must only be used when the assistant
  // has just asked whether there is anything else and the caller clearly answers
  // no/that's all. It should never be used for generic mid-call transcripts.
  if (options.afterAnythingElsePrompt) {
    return /(no,? that'?s all|no,? that is all|nothing else|no more questions)/.test(
      t,
    );
  }
...
function assistantAskedAnythingElse(text) {
  const t = normalizeSpeechText(text);
  return /(anything else|anything more|is there anything else|do you need anything else|can i help with anything else)/.test(
    t,
  );
}
  ...
  async function ensureOpenAI() {
    ...
    openaiWs.on("message", (data, isBinary) => {
      ...
      if (type === "input_audio_buffer.speech_started" && streamSid) {
        // Barge-in: if the caller starts talking while a closing/hangup is only
        // scheduled (not yet executed), cancel it and let the caller finish.
        cancelPendingHangup("caller-barge-in");
