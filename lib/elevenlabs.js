...
function splitTextForSpeech(text, maxChars = 180) {
  ...
  if (!clean) return [];
  // Split only at real sentence boundaries: punctuation immediately followed
  // by whitespace and then a capital letter/digit/quote, or end of string.
  // (Previously this split on ANY .!?;: - including the period inside a
  // domain name like "abokirate.com" - then rejoined the pieces with an
  // inserted space, which reads as a pause instead of the word "dot".)
...
async function resolveElevenLabsVoiceForAgent(db, agent = {}) {
  // CRITICAL: explicit per-agent ElevenLabs IDs must always win over legacy
  // dashboard voice names (agent.voice) and stale voice_catalog_id values.
  const explicitElevenLabsVoiceId = env(
    "",
    agent.elevenlabs_voice_id || agent.elevenLabsVoiceId || "",
  );
  const legacyVoiceId = env("", agent.voice_id || agent.voiceId || "");
  const requestedVoiceId = explicitElevenLabsVoiceId || legacyVoiceId;
  const requestedCatalogId = env("", agent.voice_catalog_id || "");
  const requestedVoiceName = env(
    "",
    agent.elevenlabs_voice_name || agent.elevenLabsVoiceName || "",
  );
  const voiceProfile = env(
    "",
    requestedVoiceName || agent.voice || agent.voiceProfile || "",
  );
  ...
  if (db) {
    try {
      // 1. The exact selected ElevenLabs voice ID is the source of truth.
      if (requestedVoiceId) {
        ...
        if (byVoice?.data?.voice_id)
          return {
            ok: true,
            source: explicitElevenLabsVoiceId
              ? "elevenlabs_voice_id"
              : "voice_id",
      ...
      // 2. Catalog ID is only used when no explicit voice ID exists.
      if (!requestedVoiceId && requestedCatalogId) {
        const byId = await db
          .from("voice_catalog")
          .select(
            "id,provider,display_name,voice_id,model_id,is_active,metadata",
      ...
      // 3. Name/profile matching is last resort only.
  ...
  if (requestedVoiceId) {
    return {
      ...
      voice: {
        id: requestedVoiceId,
        provider: "elevenlabs",
        displayName: requestedVoiceName || requestedVoiceId,
...
async function fetchElevenLabsTtsResponse({
  ...
  const timeoutMs = numberEnv("ELEVENLABS_TTS_TIMEOUT_MS", 8000, 1500, 30000);
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      const timeoutErr = new Error(
        `ElevenLabs TTS timed out after ${timeoutMs}ms.`,
      );
      timeoutErr.code = "elevenlabs_tts_timeout";
