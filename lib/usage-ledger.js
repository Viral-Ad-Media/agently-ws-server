...
async function logKnowledgeSyncUsage({
  ...
  // pages_attempted / pages_failed are NOT billable — see agently-server's
  // usage-ledger.js logKnowledgeSyncUsage for why (fallback rate-matching by
  // unit alone used to double/triple-charge one scrape). Kept as metadata on
  // sync_attempt instead of separate billable events.
...
async function logRailwayRuntimeUsage({
  ...
  return insertUsageEvent({
    organizationId,
    userId,
    // AWS, not Railway. Railway-era rows keep provider "railway" so the
    ...
    // Forwarded so runtime usage carries a real internal cost. Previously this
    // was dropped, every runtime event stored unit_cost_usd = null, and the
    // margin rule priced it at zero.
    unitCostUsd: unitCostUsd == null ? null : safeNumber(unitCostUsd),
