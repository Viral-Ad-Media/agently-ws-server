...
async function insertUsageEvent(event) {
  ...
  const payload = {
    ...
    estimated_cost_usd: event.estimatedCostUsd ?? event.estimated_cost_usd ?? null,
  ...
  try {
    ...
    if (!/record_billing_usage_event|function .* does not exist|Could not find the function/i.test(message)) {
      throw rpcError;
    }
  }
...
function openAIComponentRows(normalized) {
  return [
    { key: "input", eventSuffix: "input_tokens", quantity: normalized.input_tokens, unitDetail: "input_tokens" },
    { key: "output", eventSuffix: "output_tokens", quantity: normalized.output_tokens, unitDetail: "output_tokens" },
    { key: "cached_input", eventSuffix: "cached_input_tokens", quantity: normalized.cached_input_tokens, unitDetail: "cached_input_tokens" },
    { key: "audio_input", eventSuffix: "audio_input_tokens", quantity: normalized.audio_input_tokens, unitDetail: "audio_input_tokens" },
    { key: "audio_output", eventSuffix: "audio_output_tokens", quantity: normalized.audio_output_tokens, unitDetail: "audio_output_tokens" },
...
async function logOpenAIUsage({
  ...
  if (cachedInputTokens != null) normalized.cached_input_tokens = safeNumber(cachedInputTokens);
  if (audioInputTokens != null) normalized.audio_input_tokens = safeNumber(audioInputTokens);
  if (audioOutputTokens != null) normalized.audio_output_tokens = safeNumber(audioOutputTokens);
  const total =
    normalized.input_tokens +
    normalized.output_tokens +
    normalized.cached_input_tokens +
  ...
  if (!exactUsage) {
    return insertUsageEvent({
      ...common,
      eventType: String(eventType || "openai_text_tokens").includes("usage_missing")
        ? eventType
