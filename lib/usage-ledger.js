...
async function requireWalletCharge(sb, usageEventId, organizationId, billable) {
  ...
  const { data: charge, error: chargeError } = await sb
    ...
    .select("id,customer_charge_usd,wallet_transaction_id")
  ...
  if (
    safeNumber(charge.customer_charge_usd, 0) > 0 &&
    !charge.wallet_transaction_id
  ) {
    const error = new Error(
      "Realtime usage was priced but not deducted from the wallet.",
    );
    error.code = "BILLING_WALLET_POST_FAILED";
    throw error;
  }
  ...
  return {
    chargeId: charge.id,
    customerChargeUsd: safeNumber(charge.customer_charge_usd, 0),
