...
async function makeOutboundCall({
  ...
  if (detection) {
    ...
    const timeout = intEnv("OUTBOUND_MACHINE_DETECTION_TIMEOUT", 0, 0);
    if (timeout > 0) params.MachineDetectionTimeout = String(timeout);
  }
  if (recordingEnabled && apiUrl) {
    params.Record = "true";
    params.RecordingChannels = "dual";
    params.RecordingStatusCallback =
      recordingStatusCallbackUrl || `${apiUrl}/api/twilio/recording-status`;
...
function buildSystemPrompt(agentRow, faqs = [], knowledgeChunks = []) {
  ...
  const isOutbound =
    String(agentRow?.direction || "").toLowerCase() === "outbound";
  const lines = [
