...
function buildSystemPrompt(agentRow, faqs = [], knowledgeChunks = []) {
  ...
  const knowledgeText = (knowledgeChunks || []).length
    ? knowledgeChunks.map((chunk, index) => `[Relevant ${index + 1}] ${chunk.content || chunk}`).join("\n\n---\n\n")
    : "";
  ...
  const callPurposes = Array.isArray(agentRow?.call_purposes)
    ? agentRow.call_purposes.map((item) => `- ${String(item || "").trim()}`).filter((item) => item !== '- ')
    : [];
  ...
  lines.push('When the conversation is ending, add a final JSON line exactly like {\"captured\": {\"name\": \"...\", \"phone\": \"...\", \"email\": \"...\", \"reason\": \"...\"}}.');
  return lines.join("\n\n");
}
