...
const UA = "Mozilla/5.0 (compatible; AgentlyBot/1.0; +https://agently.ai/bot)";
...
async function checkKnowledgeBase(db, kb) {
  ...
  const mode = String(kb.change_monitoring_mode || "notify_only");

  if (mode === "auto_rescrape") {
    // Issue 4(k) — queue the re-scrape and tell them it started.
    await db
      .from("knowledge_discovered_pages")
...
async function runCycle() {
  ...
  const { data: bases, error } = await db
    .from("knowledge_bases")
    .select(
      "id,organization_id,change_monitoring_enabled,change_monitoring_interval_hours,change_monitoring_mode,last_change_check_at",
    )
    .eq("change_monitoring_enabled", true)
    .limit(200);

  if (error) {
    log("cycle query failed:", error.message);
    return;
  }

  for (const kb of bases || []) {
    const intervalMs =
      Number(kb.change_monitoring_interval_hours || 24) * 60 * 60 * 1000;
...
function start() {
  ...
  runCycle().catch((e) => log("initial cycle:", e?.message || e));
  timer = setInterval(() => {
