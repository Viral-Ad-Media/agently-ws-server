...
async function checkKnowledgeBase(db, kb) {
  ...
  // Was hardcoded to always "auto_rescrape", silently ignoring the tenant's
  // actual setting — the notify_only branch below was complete and correct,
  // just unreachable. Now honors whatever the toggle actually saved.
  const mode = kb.change_monitoring_mode === "notify_only"
    ? "notify_only"
    : "auto_rescrape";
